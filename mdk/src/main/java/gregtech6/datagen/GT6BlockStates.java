...
public final class GT6BlockStates extends BlockStateProvider {
    ...
    protected void registerStatesAndModels() {
        ...
        addStaticStorages(); // task p26-storage-static-batch
        addAdvancedCraftingTable(); // task p24-act-machine
        // task p21-paintable-tint-render: the datagen-JVM census half — 149 machine blocks x
        // 3 models (the six ULV rows joined at task p28-c-ulv-machine-ladder; the roll ladders
        // and the six P29 W1 process families joined at their owning cards; the seven eu-hu
        // families at task p29-w1-eu-hu-families; the six exotic-energy families at task
        // p29-w2-exotic-energy), // matching the paintableBlockArray() client registration census
        // (the offline JUnit half walks the generated tree and pins the same 447; the ACT
        // rides its own single-state model OUTSIDE the paint-array census — the
        // GTAdvancedCraftingTableBlock carries no ACTIVE/RUNNING payload, and the
        // family-wide paint extension stays pooled).
        LOGGER.info("GT6 machine paint tint: {} machine models tinted (149 blocks x 3 + the ACT single-state model, addOven/addMachine/addDryer/addDistillery/addCanner/addKineticTrio/addPress/addExtruder/addUlvLadder/addRollLadders/addProcessMachines/addEuHuFamilies/addExoticFamilies/addAdvancedCraftingTable)", mMachineTintModels);
        }
    }        addMultiBlocks();
        addBarrel();
        addEnergySource();
