...
public final class GTWrenchGridRenderer {
	...
	/** RenderSystem.lineWidth(3) — isolated so the headless server never even links it. */
	private static void RenderSystem_setLineWidth() {
	...
	private static void drawIcons(PoseStack aPoseStack, MultiBufferSource aBuffers, byte aFace, float[] aOrigin,
		...
		float tCell = GTWrenchGridTables.CELL_16 / 16.0F;
		...
		for (int tRow = 0; tRow < 3; tRow++) {
			for (int tCol = 0; tCol < 3; tCol++) {
				...
				int tColor = tCellSide == aHover ? COLOR_HOVER : COLOR_DIM;
				float tU0 = GTWrenchGridTables.CELL_ORIGIN_16[tCol] / 16.0F + tInset;
				float tU1 = (GTWrenchGridTables.CELL_ORIGIN_16[tCol] + GTWrenchGridTables.CELL_16) / 16.0F - tInset;
				float tV0 = GTWrenchGridTables.CELL_ORIGIN_16[tRow] / 16.0F + tInset;
				float tV1 = (GTWrenchGridTables.CELL_ORIGIN_16[tRow] + GTWrenchGridTables.CELL_16) / 16.0F - tInset;
				// PNG u runs with the table u axis; PNG v runs against the table v axis
				// (PNG top = the v-near end) — the pairing is fixed, only the emission
				// order flips to keep the quad CCW from outside
				float[][] tQuads = tFlip
				...
				VertexConsumer tBuffer = aBuffers.getBuffer(
						RenderType.text(new ResourceLocation(GTRenderModelListener.MOD_ID, tIcon.texturePath)));
