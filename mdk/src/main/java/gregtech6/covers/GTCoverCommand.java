 ...
 * FakePlayerFactory). The right-click install and the crowbar dismantle run through
 * the ICoverableTE dispatches; the hoe-class crowbar substitute is a real hoe in the
 * fake player's hand, so the ToolActions classification path is what gets exercised:
 *
 * <ul>
 * <li>{@code install [<pos>] [<side>]} — setCoverItem(side, plate_iron stack, player,
 ...
 *     covers NBT keys and the render snapshot mask (acceptance ③ server evidence);</li>
 ...
 * </ul>
 *
 * <p>The GTOvenBlock.use wiring (right-click install in-game) is a 3-line append to a
 * file outside this card's FILES_SCOPE — the dispatch itself lives in
 * {@link ICoverableTE#onCoverUse} and is unit-tested; the wiring point is documented
 * there and handed to the merge review.
 */
...
public final class GTCoverCommand {
	...
	public static void onRegisterCommands(RegisterCommandsEvent event) {
		LiteralArgumentBuilder<CommandSourceStack> tCover = Commands.literal("gt6cover")
			...
			.then(Commands.literal("install")
				.executes(context -> install(context.getSource(), null, Direction.UP))
				.then(Commands.argument("pos", BlockPosArgument.blockPos())
					.executes(context -> install(context.getSource(), BlockPosArgument.getLoadedBlockPos(context, "pos"), Direction.UP))
		...
		LOGGER.info("Registered GT6 cover acceptance command /gt6cover (install|dismantle|check)");
	}
	...
	private static TileEntityOvenHost ovenHostAt(CommandSourceStack source, BlockPos pos) {
		ServerLevel tLevel = source.getLevel();
		BlockPos tTarget = pos != null ? pos : BlockPos.containing(source.getPosition());
		return tLevel.getBlockEntity(tTarget) instanceof TileEntityOven tOven ? new TileEntityOvenHost(tOven, tTarget) : null;
	}

	private record TileEntityOvenHost(TileEntityOven oven, BlockPos pos) {
	}

	/** Install the iron-plate cover on the face; asserts the NBT keys + snapshot mask. */
	private static int install(CommandSourceStack source, BlockPos pos, Direction side) {
		TileEntityOvenHost tHost = ovenHostAt(source, pos);
		if (tHost == null) {
		...
		ItemStack tCoverStack = new ItemStack(ForgeRegistries.ITEMS.getValue(new net.minecraft.resources.ResourceLocation("gt6", "plate_iron")));
		if (tCoverStack.isEmpty()) {
			source.sendFailure(Component.literal("gt6:plate_iron not registered"));
			return 0;
		}
		boolean tOk = tHost.oven().setCoverItem(tSide, tCoverStack, FakePlayerFactory.getMinecraft(source.getLevel()), false, true);
		CompoundTag tCoversTag = new CompoundTag();
		tHost.oven().writeCoversToNBT(tCoversTag);
		GTCoverRenderSnapshot tSnapshot = snapshotOf(tHost.oven());
	...
	private static int dismantle(CommandSourceStack source, BlockPos pos, Direction side) {
		...
		long tDamage = tHost.oven().onCoverToolClick("", null, new ItemStack(Items.WOODEN_HOE), tSide, false);
		...
		String tReport = String.format("GT6 cover dismantle %s face %s at %s: toolDamage=%d, drops=%s, coversNbt=%s, store=%s",
				tHost.oven(), side, tHost.pos().toShortString(), tDamage,
				tDrops.stream().map(t -> t.getItem().getCount() + "x " + t.getItem().getItem()).toList(),
				tCoversTag.getCompound(ICoverableTE.NBT_COVERS), tHost.oven().getCovers() == null ? "null" : "alive");
		if (tDamage != 10000 || tDrops.isEmpty() || tHost.oven().getCovers() != null) {
	...
	private static int check(CommandSourceStack source, BlockPos pos) {
		...
		source.sendSuccess(() -> Component.literal(String.format(
				...
				tHost.pos().toShortString(), tHost.oven().getCovers() == null ? "null" : "alive",
	...
	private static GTCoverRenderSnapshot snapshotOf(TileEntityOven aOven) {
		return aOven.getModelData().get(GTModelProperties.RENDER_SNAPSHOT) instanceof GTCoverRenderSnapshot tSnapshot ? tSnapshot : null;
	}
}
