...
- **池拉齐并发（2026-09-08 用户裁定）：coder 池未满即从池拉活**。冻结串行线只表达依赖，
  ...
  （或冲突点显式写进合并序）。
- **金字塔拆分调度（2026-09-12 用户裁定，P28 起常设）**：大卡一律拆小卡分层执行，
  测试成本集中到塔尖——
  ①**architect 拆卡即分层**：每张大卡 = N 张底层实现小卡（文件域互斥、可并行）
  + 1 张顶层集成卡；拆卡产出里写明金字塔结构（底层卡清单+集成卡 SPEC）。
  ②**底层 coder 只改代码**：门禁=双腿编译确认（compileJava+compileTestJava，
  约 1-2 分钟）；**禁跑 cleanTest/test/RCON**；有 datagen 面的卡可跑 runData 产 JSON
  （产物生成非测试）。产分支 work/<大卡>-p<i>，不上 main。
  ③**顶层 coder（每塔一个）**：主会话收齐全部分支后派发，prompt 附底层分支清单；
  顶层把全部分支 merge 进集成分支 work/<大卡> → 跑全门禁（cleanTest 双腿+RCON 链
  +runData）→ 修复问题（直接在集成分支）→ 交付。
  ④**review-merge 只审集成分支**，一次合入 main——main 始终全绿，审查一次覆盖
  整座塔。
  ⑤**分发调度全在主会话**：subagent 无权嵌套派发 subagent；底层分支清单、合并序、
  冲突预警由主会话写进顶层卡 prompt。
  ⑥适用边界：小型修复/单文件卡不必分层，直接单卡全门禁照旧；是否分层由 architect
  拆卡时声明、主会话裁定。
