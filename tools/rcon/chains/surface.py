#!/usr/bin/env python3
"""surface — the surface-plants + fallen-woods live acceptance chain (task
surface-blocks; the trees /place-feature form).
...
no placement modifiers, no rarity roll, deterministic at a known origin):
  ...
  B the destroy-drop faces: setblock + `air destroy` drops the block item —
    ...
    the rocks_sticks item-entity idiom).
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/surface.py
"""
...
# ---------------------------------------------------------------- T: teardown
# teardown-fix: the band clear was ONE /fill of 129x24x19 = 58824 blocks —
