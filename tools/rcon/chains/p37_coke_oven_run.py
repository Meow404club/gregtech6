#!/usr/bin/env python3
"""p37-coke-oven-run — the Coke Oven LIVE RUN chain (task p37-chains-coke-oven-run,
...
Chain semantics (the card path):
  ...
  A the multiblock form arm (the p16_form_scaffold positive-arm semantics,
  ...
  C the s4-6 laser-gas census arm (pure census, no behavior assertion): the two
    ...
    the p32_qu_usb give-chest form (RCON has no player, /give is unreachable;
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/p37_coke_oven_run.py
"""
...
CHAIN = Chain(
    name="p37-coke-oven-run p37_coke_oven_run",
    slug="p37coke",
