#!/usr/bin/env python3
"""tag-input-machine-fallback — the machine-input tag fallback live chain
(declarative framework, the drying_rows shape).

Chain semantics (task tag-input-machine-fallback ACCEPTANCE 2 — the RCON
...
foreign 物品命中 tag fallback + 精确分支回归臂"):
  ...
  D the IRON_INGOT ring arm (the red-chains-modernize reshape — the former red,
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/tag_input_machine_fallback.py
"""
...
CHAIN = Chain(
    name="tag-input-machine-fallback",
    slug="taginput",
    sites=gt6world.declare_sites(SHREDDER_SITE, RING_IRON_SITE, NEG_DUST_SITE),
    preferred_ports=(26108, 26118),      # this card's pinned rcon/query pair (after dyechemical 26106/26116)
    steps=steps,
)
