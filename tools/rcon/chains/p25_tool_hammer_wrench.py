#!/usr/bin/env python3
"""p25-tool-hammer-wrench — the hammer + wrench /give live acceptance chain (the
p24_act give-arm shape: RCON sessions have no player, so the headless give is
`item replace block <chest> container.N with <stack>` + a `data get block` probe).

Chain semantics (task p25-tool-hammer-wrench ACCEPTANCE ③):
...
addressable headless (the p24_act ruling).

passes=2 is the idempotency proof.

Run:  GT6_SESSION=off python3 tools/rcon/chains/p25_tool_hammer_wrench.py
"""
...
steps += [
    ...
    # the data-get rendering drifts per node (the p16pchk s17 node_expects rule):
...
CHAIN = Chain(
    name="p25-tool-hammer-wrench",
    slug="p25toolhammerwrench",
    sites=gt6world.declare_sites(SITE),
    preferred_ports=(26108, 26118),      # this card's pinned rcon/query pair (the 2610x segment, after p24grassblock 26107/26117)
    steps=steps,
)
