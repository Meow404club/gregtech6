#!/usr/bin/env python3
"""circuits_craft — the circuits C-column crafting chain (task
circuits-crafting-c RCON: the 26 integrated-circuit rows + the
...
transcriptions). What a dedicated server node adds on top is the LIVE face:
  ...
  A the recipe load: /reload re-parses the datapack — a missing/unparseable
    ...
    the chisel_decalcify give-face ruling).
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/circuits_craft.py
"""
...
# the fresh z=290 band (south of circuits_parts z=284, north of parts
...
steps = [
    ...
    # (the chisel_decalcify give-face ruling). The load face is /reload: a missing/unparseable
...
CHAIN = Chain(
    name="circuits_craft circuits-craft",
    slug="circuitscraft",
