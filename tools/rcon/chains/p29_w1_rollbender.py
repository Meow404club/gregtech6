#!/usr/bin/env python3
"""p29_w1_p29_w1_rollbender — the Roll Bender RU ladder live acceptance chain (task
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/p29_w1_rollbender.py
"""
...
steps = [
    ...
    Step(f"setblock {MILL_T1_P} gt6:rollbender_t1", expect="Changed the block", sleep=1.0),
    Step(f"gt6machine rollbender_t1 input 1 {MILL_T1_P}",
         expect="GT6 rollbender_t1 input: 1x plate_iron into slot 0",
         node_expects={"1.21.1": "GT6 rollbender_t1 input: 1x gt6:plate_iron into slot 0"}),
    # the wall: 8 < mInputMin 16 — no recipe ever binds
    Step(f"gt6machine rollbender_t1 inject 60 8 {MILL_T1_P}", expect="progress=0/0"),
    Step(f"gt6machine rollbender_t1 check {MILL_T1_P}", expect="progress=0/0"),
    # the control leg: the packet size was the red, not the rig
    Step(f"gt6machine rollbender_t1 inject 60 64 {MILL_T1_P}", expect=OUT, node_expects=OUT),
...
steps += [
    ...
    Step(f"gt6machine rollbender_t1 check {MILL_T1_P}", expect="minIn=16 recIn=32 maxIn=64"),
