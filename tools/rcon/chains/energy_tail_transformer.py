#!/usr/bin/env python3
"""energy-tail-machines — the transformer ladder live acceptance (sweep group
energy_tail; the fresh z=420 band, x520..538 — x/z-disjoint from every
registered band: the P28 strip sits z40..64, the ignition band z452..478).

  THE SINK (the battery_charge lesson): the EU sink is the BatteryBox
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/energy_tail_transformer.py
"""
...
def empty_battery(pos, battery):
    ...
    the battery_charge EMPTY_BATTERY merge form, the carrier fork)."""
    return {
...
CHAIN = Chain(
    name="energy_tail_transformer electric transformer ladder",
    slug="energytailtransformer",
