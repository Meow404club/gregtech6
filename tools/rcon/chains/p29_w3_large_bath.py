#!/usr/bin/env python3
"""p29-w3-large-bath — the Large Bathing Vat chain (task p29-w3-large-12, the
p29_w3_large12 sweep group; Loader_MultiTileEntities.java:1233).

  TU self-generating, window 1..16, parallel 64, NO_CONSTANT_POWER; the 5x5x2 SS wall
  box; the bath.json wool row (white wool + water 1000 -> 4 string, eUt 16, dur 128) —
  the 2048-progress bar at the 1/tick self-generation pace.

acceptance arms this machine carries:
  1 form all-green + the wrong-part rejection
  3 the NO_CONSTANT_POWER cycle (the stopped-toggle gap parks the bar, the restart resumes)

passes=2 is the idempotency proof.
Run:  GT6_SESSION=off python3 tools/rcon/chains/p29_w3_large_bath.py
"""
...
CTRL = gt6world.Site(432, 65, 340)
RIG = gt6world.Site(432, 64, 340)   # the under-controller cell: FREE (the reserved site)
C, R = F(CTRL), F(RIG)
WRONG = gt6world.Site(432, 65, 342)
W = F(WRONG)
...
FEED = merge([("minecraft:white_wool", 1)], fluid=("minecraft:water", 1000))
...
CHAIN = Chain(
    name="p29-w3-large-bath p29_w3_large12",
    slug="p29w3largebath",
