#!/usr/bin/env python3
"""runtime-smoke — the ADR-P15-4 runtime smoke, node-agnostic by construction.
...
is the 1.21.1 counterpart of the loop_closure semantics, prefaced by the
registration face (task rcon-dual-gate spec ③).
  ...
  M basic machine: the shredder arm (GTMachineCommand canon) on the DATA PLANE
    (the red-chains-modernize reshape). The shredder is a menu-less carrier
    (the MenuType registration is gone since the mui-a-menu-deregistration
  ...
  D distillation loop closure (the loop_closure arm, verbatim semantics):
  ...
  K keepFilter round-trip (runtime evidence 4b, live face): the logistics tank
    ...
    keepfilter_reboot_probe.py — this chain's faces are shared code, the
    known 21.1 codec delta only bites across a reboot.

Run:  python3 tools/rcon/chains/runtime_smoke.py [--node 1.21.1-neoforge]
"""
...
steps += [
    ...
    # data plane the P25/W1 chains drive) — no fakesource flip, the global
...
steps += [
    ...
    # 25600 HU / 16 per tick = 1600 ticks ≈ 84 s. poll-to-expect (the loop_closure
