#!/usr/bin/env python3
"""machines-bp — the Burner Mixer + Plantalyzer live chain (task
machines-burner-plantalyzer, RCON group machines_bp):
  ...
  A the Burner Mixer (20521, the RU ignition family, RM.BurnMixer) — the :220
    ...
    the /gt6energy RU rig (the sander form, the SBIT_D bottom face) ->
  ...
  C the Plantalyzer (20531, the EU 5-ladder, RM.PLANTALYZER) — the DECLARED-EMPTY
    ...
    fills water (the gui fluid seat's live carrier) -> the open smoke arm
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/machines_bp.py
"""
...
steps = [
    ...
    Step(f"gt6machine plantalyzer fluid fill up minecraft:water 1000 {F(PL)}",
         expect="filled 1000/1000 L of minecraft:water (ACCEPTED), input tanks hold 1000 L"),  # the single tank-in face: the gui fluid seat's live carrier
...
CHAIN = Chain(
    # the name carries the group key: sweep --group machines_bp matches
    name="machines-bp machines_bp",
    slug="machinesbp",
    sites=gt6world.declare_sites(BM, BM_RIG, BM2, BM3, BM4, PL, PL_RIG),
    preferred_ports=(26672, 26682),      # this card's pinned rcon/query pair (after guibmach 26652/26662)
    steps=steps,
)
