#!/usr/bin/env python3
"""act-machine — the Advanced Crafting Table e2e (declarative framework, the
canner_refill shape; C1 GUI-free, everything rides /gt6act).

Chain semantics (task act-machine ACCEPTANCE — "give selector → place ACT → fill
存料带 + 塞 selector → compute 断言 pattern+产出 → craft 断言消耗+产出 → stat; 开关
翻转臂（mBlocked16/36 字段）"):

  A the give arm (the chisel_decalcify headless-give shape): one chest + `item replace
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/act.py
"""
...
# the ACT site on the 410 band (clear of cannerrefill's 400 band)
...
steps += [
    ...
    # namespace drift, the distillery "integrated_circuit" precedent)
...
CHAIN = Chain(
    ...
    preferred_ports=(26109, 26119),      # this card's pinned rcon/query pair (the 2610x segment, after cannerrefill 26108/26118)
    steps=steps,
)
