#!/usr/bin/env python3
"""drying-rows-backfill — the DRYING backfill round-trip chain (declarative framework).

Chain semantics (task drying-rows-backfill ACCEPTANCE — "Dryer 新行 round-trip 抽样
...
new-row round-trips through the P16 backfill:
  ...
  B three water-family arms (the aqua-fluids six, Loader_Recipes_Chem.java:526-532):
    fill east gt6:<fluid> <in> → inject (the doInject driver, the dryer-family arm)
  ...
  C the ice-family arm (:521 snowball — the one ice row whose input is a vanilla item, so
    ...
    per the dryer-family census, inject 4000 overshoots with the idle-stall backstop)
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/drying_rows.py
"""
...
steps += [
    ...
    # the same flip loop_closure took at its :75 arm
