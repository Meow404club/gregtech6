#!/usr/bin/env python3
"""keepfilter_reboot_probe — the keepFilter SAVE/LOAD face, live (evidence 4b).

The P12 keepFilter chain and the runtime_smoke K phase cover the LIVE and the
...
line. Run per node and diff:
  ...
  python3 tools/rcon/chains/keepfilter_reboot_probe.py [--node 1.21.1-neoforge]
...
# .../tools/rcon/chains/keepfilter_reboot_probe.py — parents[1] is tools/rcon
# (the module imports), parents[3] is this worktree's root (the review handoff of
# 2026-09-04: the hardcoded rcon-dual-gate worktree path died with that
...
def main():
    ...
    node = args.node
    slug = f"kfr{'1211' if '21.1' in node else '1201'}"
