#!/usr/bin/env python3
"""p34_bee_recipes — the Bumbliary pair crafting + hive-carry live chain (task
p34-bumbliary-recipes, acceptance ④: craft 冒烟臂; acceptance ②: 挖取野生蜂窝得箱).
...
Chain semantics:
  ...
  A THE RECIPE LOAD: /reload re-parses the datapack — the two new rows
    ...
    (the p33_circuits_craft arm shape; a missing/unparseable row is a loud parse
  ...
  C THE LOOT WALK (the p33_bees_c arm, now carrying the ③ box): the merged
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/p34_bee_recipes.py
"""
...
# the fresh band: x-disjoint from the p33_bees_c band 479..520 with margin — the ACT
# column at z=144 and the loot-walk cell at z=128 (setblock-driven rigs, no forceload:
# the /place feature arm belongs to the p33_bees_c chain, this card only mines).
...
CHAIN = Chain(
    name="p34-bumbliary-recipes p34_bee_recipes",
    slug="p34beerecipes",
