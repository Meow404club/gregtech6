#!/usr/bin/env python3
"""p33_bees_a — the bumblebee item-domain chain (task p33-bees-lv3-a-items RCON:
the 8-face princess/drone/queen/dead family live-registered + the gt.bumble gene
carrier face).

  A THE REGISTRATION CENSUS (the p32_qu_usb chest form): one chest takes the 8
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/p33_bees_a.py
"""
...
# p32_qu_usb band z=348 and the p33_gui_distill band z=356 (x-disjoint columns).
CHEST = "384 65 360"
C = CHEST
...
def write_genes(code):
    ...
    same keys — the GT6BatteryItem/p32_qu_usb fork)."""
    return {
...
CHAIN = Chain(
    name="p33-bees-lv3-a-items p33_bees_a",
    slug="p33beesa",
