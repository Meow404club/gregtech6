#!/usr/bin/env python3
"""crucible-ladder — the crucible 8-material ladder live acceptance chain (task
distill-crucible ACCEPTANCE ③④: 8 档坩埚 census（17302..17312 全在 + ACIDPROOF 列）+ the
SS 坩埚 form + 熔炼一臂（the P26 链回归 on the new rung）).
  ...
  C the melt arm (the P26 crucible_multiblock regression form): 4U gold
...
Run:  python3 tools/rcon/chains/crucible_ladder.py --node 1.20.1-forge
      python3 tools/rcon/chains/crucible_ladder.py --node 1.21.1-neoforge
"""
...
CHAIN = Chain(
    name="distill_crucible crucible_ladder",
    slug="crucibleladder",
