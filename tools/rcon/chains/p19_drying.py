#!/usr/bin/env python3
"""p19-drying-rows-backfill-2 — the DRYING backfill round-trip chain (declarative framework).

Chain semantics (task p19-drying-rows-backfill-2 ACCEPTANCE — "矿物行+盐行+clay→terracotta
...
(no new command arms), the new-row round-trips of the p19 backfill:
  ...
  A the rig: dryer (FACING north, no fuel) + the feed hopper ABOVE it + the extraction
    hopper BELOW it. Two /data merges arm the rig (the p16_side_io:112 precedent):
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/p19_drying.py
"""
...
# sites (p16-drying used x=340, aqua x=300).
...
CHAIN = Chain(
    name="p19-drying-rows-backfill-2",
    slug="p19drying",
