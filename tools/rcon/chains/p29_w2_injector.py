#!/usr/bin/env python3
"""p29-w2-eu-core injector — the Injector family chain (task p29-w2-eu-core-5tier
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/p29_w2_injector.py
"""
...
steps = [
    ...
    phase("B: the T1 smoke-row run — stick + flint -> arrow"),
    Step(feed_merge(T1F)["1.20.1"], expect="Modified block data", node_cmds=feed_merge(T1F)),
    Step(f"gt6machine wiremill inject 24 32 {T1F}", expect=out_report("arrow")["1.20.1"], node_expects=out_report("arrow")),
    Step(f"gt6machine wiremill check {T1F}", expect="progress=0/0"),

    phase("C: the T5 packet-edge arm — 4095 dead below the floor, 8192 runs"),
    ...
    Step(feed_merge(T5F)["1.20.1"], expect="Modified block data", node_cmds=feed_merge(T5F)),
    Step(f"gt6machine wiremill inject 8 8192 {T5F}", expect=out_report("arrow")["1.20.1"], node_expects=out_report("arrow")),
...
CHAIN = Chain(
    name="p29_w2_eu_core injector",
    slug="p29w2eucore_injector",
