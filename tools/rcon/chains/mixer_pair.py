#!/usr/bin/env python3
"""mixer-pair — the SHARED-MAP 对拍 chain (task eu-hu-families ACCEPTANCE ①):
...
kitchen_pot live-proven item set: 6x dust_stone + 2x dust_sand + 1x dust_small_clay
...
progress = half speed / 2× the energy-time, NOT a 2× speed-up):
  ...
  (attribution CORRECTED by task hu-tu-piggyback — the original "RU machines
...
1.20.1 `Count:6b` vs 1.21.1 `count:6`). pass 2 = idempotency. Run:
  GT6_SESSION=off python3 tools/rcon/chains/mixer_pair.py
"""
...
# kitchen_pot chain's live-proven ids)
def cfoam_merge(pos):
    return {
...
CHAIN = Chain(
    # the name carries the band key: sweep --group eu_hu matches this member
    # substring and joins the whole cluster (the select_groups member-substring form)
    name="mixer-pair eu_hu",
    slug="mixerpair",
