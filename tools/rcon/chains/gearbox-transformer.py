#!/usr/bin/env python3
"""gearbox-transformer — the GearBox split / Transformer torque acceptance chain.

Chain semantics (task gearbox-transformer acceptance b, ports 25712/25722):
  ...
  C the gear-explosion arm: rig(RU 64 > VMAX[0]=16) -> gearbox (gears=28), placed
    ...
    gears: scrap + (tCount-1) gear legs drop (the gear item is the gear-items
...
Run:  python3 tools/rcon/chains/gearbox-transformer.py
"""
...
CHAIN = Chain(
    ...
    steps=[
        ...
        Step(f"gt6machine shredder input 32 {F(SHRED_N)}", expect="cobblestone into slot 0"),
        Step(f"gt6machine shredder input 32 {F(SHRED_S)}", expect="cobblestone into slot 0"),
