#!/usr/bin/env python3
"""autoclave — the TU Autoclave chain (task hu-tu-piggyback, group
hu_tu; the TU single :1655 over the JSON-poured autoclave.json smoke row —
1 kelp + 1 bone meal + water 500 -> 2 kelp, eUt 16, duration 128 — the TWO-ITEM
  ...
  (the 17-包 overcharge wall rides the coagulator chain's throwaway arm.)

passes=2 is the idempotency proof.
Run:  GT6_SESSION=off python3 tools/rcon/chains/autoclave.py
"""
...
steps = [
    ...
    # input stacks ride the data-merge inventory form (the mixer_pair shape —
