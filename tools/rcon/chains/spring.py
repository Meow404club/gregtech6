#!/usr/bin/env python3
"""spring — the bedrock-spring live chain (the bedrock_ore chain shape).
...
pins the faces that hold at EVERY seed:
  ...
  A the live REGISTRATION face (the id686 lesson: a registration face needs live proof):
    ...
    task fluid-spring spec ①) plus the p5 gt6:natural_gas_block face. A registered id
  ...
  B the gate face, DETERMINISTIC: a body WITHOUT the bedrock floor cannot pass the
    ...
    t2 single-flake precedent). The DECISION-level positive proof of natural
...
(chunk (4,8) — x/z-disjoint from the bedrock_ore band (z96..111) and the nether
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/spring.py
"""
