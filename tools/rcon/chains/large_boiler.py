#!/usr/bin/env python3
"""large-boiler — the Large Boiler multiblock acceptance chain (declarative framework).

Chain semantics (task large-boiler ACCEPTANCE ②):
  ...
  A the positive chain — the FULL five-pipe-hole loop:
    ...
    of the energy-relay chain) → the era cadence: water+HU rounds then the THIRD and
...
P34 MIGRATION (task boiler-tick-step, the ops-neo-tick-primitive ruling:
"large-boiler 迁移归后续卡=认可"): every wall-clock timing arm — the sleeps and
...
Run:  python3 tools/rcon/chains/large_boiler.py
"""
