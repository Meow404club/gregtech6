#!/usr/bin/env python3
"""gui-distill-tower — the distillation-tower GUI acceptance chain (task
gui-distill-tower ACCEPTANCE ②, the act C2 open-smoke shape).
...
Run:  python3 tools/rcon/chains/gui_distill_tower.py --node 1.20.1-forge
      python3 tools/rcon/chains/gui_distill_tower.py --node 1.21.1-neoforge
"""
...
# TAKEN by pocket_ring (x384..386, the give chest + walk rig), the
...
CHAIN = Chain(
    name="gui_distill_tower",
    slug="guidistill",
    sites=gt6world.declare_sites(
        gt6world.Site(385, 63, 356, dx=2, dy=10, dz=3),   # the tower body + arm column
    ),
    preferred_ports=(26129, 26139),      # this card's pinned rcon/query pair (after act 26109/26119)
    steps=steps,
)
