#!/usr/bin/env python3
"""canner-machine — the Canning Machine refill e2e + the dynamic arms (declarative
framework, the side_io shape).

Chain semantics (task canner-machine ACCEPTANCE 3 — "place canner→注染液→input
...
Step.node_expects (the pattern_checker.py:114 precedent): the 1.20.1
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/canner_refill.py
"""
...
# the four canner sites on the 400 band (clear of dyechemical's 386 band)
...
# the barrel-item payload per node (the nbt_rebind key-shape ruling): an EMPTY
...
CHAIN = Chain(
    ...
    preferred_ports=(26108, 26118),      # this card's pinned rcon/query pair (the 2610x segment, after dyechemical 26106/26116)
    steps=steps,
)
