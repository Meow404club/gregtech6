#!/usr/bin/env python3
"""chains-anvil — the stone/blackstone anvil live-smoke chain (task
chains-anvil, the P37-d s4-2 gap closure).

The gap (research.gap-scan.s4-2): both anvil rows are registered
...
but had zero live smoke. Two arms, the shortest Java-free live face:
  ...
  B THE DROPSELF LOOT 到账: each row's loot table (GT6LootTables
    ...
    read back from the container NBT (the loot_inject proven face).
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/anvil_run.py
"""
...
CHAIN = Chain(
    ...
    # session-ports table (the cover_hosts form).
