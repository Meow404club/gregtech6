#!/usr/bin/env python3
"""p32-ignition-gate — the fusion start-LU ignition gate live acceptance (sweep group
p32_ignition; the p31_fusion rig geometry, shifted +52 x into the fresh x490..514 band):

  the same 17198 controller rig (27 PUs / 50 vents / 48+6 walls / three OCTAGONS rings),
  now with the :1242 NBT_SPECIAL_IS_START_ENERGY gate LIVE (task p32-ignition-gate, the
  ...
  C THE START (acceptance ①): at ~tick 1261 the gate opens — active 1b, the ring
    ...
    the p31_fusion oracle), and ~315 ticks later the run completes: active 0b,
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/p32_ignition.py
"""
...
# ---------------------------------------------------------------------------
# the structure placement, generated from the transcribed masks (the p31_fusion
...
def selector_merge(slot, config):
    """The selector circuit stack (the p31_fusion fork)."""
    return {
...
steps = [
    ...
    *structure_steps(),
    ...
    # the 101st EU packet explodes it mid-run, the p31_fusion oracle)
...
CHAIN = Chain(
    name="p32-ignition-gate p32_ignition",
    slug="p32ignition",
