#!/usr/bin/env python3
"""p34_machines_bc — the Bumblelyzer + Crystallisation Crucible live chain (task
p34-machines-bumblelyzer-crucible, RCON group p34_machines_bc):
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/p34_machines_bc.py
"""
...
Z = 384  # the fresh band — z-disjoint from the p34_bumbliary_gui band z=376 and everything below
...
steps = [
    ...
    Step(f"gt6machine bumblelyzer inject 64 32 {F(BL1)}",
         expect="inject ticks=64 size=32 finalSize=null used=2048"),
    ...
    # the exact row clock: 16 EUt x 72000 t = 1152000 HU, the 16-per-tick feed lands it on
    # the last tick; the +100 overshoot absorbs any first-tick edge
    Step(f"gt6machine crystallisationcrucible inject 72000 16 {F(CC1)}",
         expect="inject ticks=72000 size=16"),
...
CHAIN = Chain(
    name="p34-machines-bumblelyzer-crucible p34_machines_bc",
    slug="p34machbc",
