#!/usr/bin/env python3
"""p37-kinetics-water-wheel — the Water Wheel RU-source acceptance chain (task
p37-chains-water-wheel, pool A6, the p12 kinetics chain form).
...
Chain semantics — the offline test's chain-closure table walked in-world:
  ...
  A the rig: water_wheel[axis=x] at (664,64,520) -> axle_wood_treated_small
    ...
    takes the RU, the p28_ulv_chain arm-A form). The wheel's consumer universe
...
Run:  python3 tools/rcon/chains/p37_kinetics_water_wheel.py
      python3 tools/rcon/chains/p37_kinetics_water_wheel.py --node 1.21.1-neoforge
"""
