#!/usr/bin/env python3
"""eu-core electrolyzer — the Electrolyzer family chain (task
eu-core-5tier ACCEPTANCE ①②④⑤): the FIRST 5-tier ladder of the port
...
TWO slots — the :708-710 MIN-2 face):
  ...
  C the type-gate LIVE arm (the cross-card-1 guard, the energy arm-E form):
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/electrolyzer.py
"""
...
steps = [
    ...
    Step(f"gt6machine wiremill inject 24 128 {T2F}", expect=None),
    ...
    Step(f"gt6machine wiremill inject 24 512 {T3F}", expect=None),
    ...
    Step(f"gt6machine wiremill inject 24 2048 {T4F}", expect=None),
    ...
    Step(f"gt6machine wiremill inject 24 4096 {T5F}", expect=None),
...
CHAIN = Chain(
    name="eu_core electrolyzer",
    slug="eucore_electrolyzer",
