#!/usr/bin/env python3
"""boule_lathe_sticklong3 — the boule consumption chain, LATHE arm (task
boule-consumption-rcon, RCON group machines_bc):

  A the CRYSTALLISATION arm — the :683 Si-boule row (the machines_bc
  ...
  B the LATHE arm — the :379 row live: boule_gt_silicon 1 -> stick_long_silicon
    ...
    transfer rides the rm_backfill hopper-feed idiom (the machine-neighbor
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/boule_lathe_sticklong3.py
"""
...
steps = [
    ...
    # the fast-forward arm (the machines_bc chain-B verbatim): 16 EUt x 72000 t =
...
CHAIN = Chain(
    name="boule-lathe-sticklong3 machines_bc",
    slug="boulelathe",
