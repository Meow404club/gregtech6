#!/usr/bin/env python3
"""bees_c — the hive LOOT live chain (task bees-lv3-c-hive-loot, acceptance ②:
worldgen 放 hive → 打破 → princess/drone/comb 掉落断言).

Chain semantics:

  A THE WORLDGEN PLACE (the bees_hive arm-C form, now covering the loot rolls): a
  ...
  B THE LOOT WALK (the drop table the scoop harvest): a /setblock hive data-merged
    ...
    (the bumbliary-recipes R2 box-drop semantics: the upstream base getDrops,
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/bees_c.py
"""
...
CHAIN = Chain(
    name="bees-lv3-c-hive-loot bees_c",
    slug="beesc",
