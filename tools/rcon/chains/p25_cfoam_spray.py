...
拆管再置回流断言；p24_pipe_owner 链回归 [0,0]"):
  ...
  E the round-trip arm (spec ⑦): spray owned + dry again, /data get block asserts the
    ...
    .BlockEntityTag) — the copy_custom_data mirror rewrite (task p28-neo-loot-copy-
    ...
    the p24_act component-envelope precedent; the value rendering "gt.foamed: 1b" is
...
# the copy_custom_data rewrite (p28-neo-loot-copy-custom-data) keeps the BlockEntityTag
# compound INSIDE the minecraft:custom_data component, so only the wire path forks
# (Step.node_cmds, the p24_act SELECTOR_REPLACE precedent; the expect is the leg-neutral
