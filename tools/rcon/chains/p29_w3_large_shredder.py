#!/usr/bin/env python3
"""p29-w3-large-shredder — the Large Shredder chain (task p29-w3-large-12, the
p29_w3_large12 sweep group; Loader_MultiTileEntities.java:1239).

  RU eff 5000, window 512..4096, parallel 64 + DURATION + NO_CONSTANT_POWER; the 5x5x3
  basin with the shredder-blade filling (the Crusher's :1239 twin); the heavy andesite
  row (andesite -> cobblestone, eUt 16, dur 512) — 64 andesite = 64 processes, the
  1048576-progress bar at the 512/t rig pace = 2048 ticks.
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/p29_w3_large_shredder.py
"""
...
CTRL = gt6world.Site(504, 65, 340)
RIG = gt6world.Site(504, 64, 340)   # directly UNDER the controller
C, R = F(CTRL), F(RIG)
WRONG = gt6world.Site(504, 65, 342)
W = F(WRONG)

MACHINE = "large_shredder"
...
FEED = merge([("minecraft:andesite", 64)])

steps = [
    phase("A: the site — the wrong-part rejection, then the all-green form"),
    Step("fill 501 60 337 507 70 352 air", expect="filled"),
    ...
    phase("B: the run — the numeric 524288 bar over the blade basin"),
...
CHAIN = Chain(
    name="p29-w3-large-shredder p29_w3_large12",
    slug="p29w3largeshredder",
