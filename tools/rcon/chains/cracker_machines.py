#!/usr/bin/env python3
"""cracker_machines — the cracker verification chain (task cracker-machines,
RCON group cracker_machines; the research.r-cracking verdict: machines and
recipe rows both landed, this chain is the live verification):

  A the STEAM_CRACKING positive run (the TRUE :368 row, the P29 W2 chain's proven
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/cracker_machines.py
"""
...
CHAIN = Chain(
    # the name carries the band key: sweep --group cracker_machines matches
    name="cracker-machines cracker_machines",
    slug="crackermachines",
