#!/usr/bin/env python3
"""p31-fusion — the Fusion Reactor chain (task p31-fusion, the p31_fusion sweep group;
Loader_MultiTileEntities.java:1242, upstream FusionReactor.java:47-126).
  ...
  - the glass ring y0 (design 5 inactive / 6 active, ONLY_ENERGY_IN) relays LU to the
    ...
    ring cell proves the face is reachable. SINCE p32-ignition-gate the gate is LIVE
...
def selector_merge(slot, config):
    """The selector circuit stack (the p31_implosion Damage-carrier fork)."""
    return {
