#!/usr/bin/env python3
"""pipe-owner — the pipe ownership live chain (declarative framework, the
dye_chemical_fluids shape).

Chain semantics (task pipe-owner ACCEPTANCE — "RCON /gt6pipe 扩 ownable 子命令
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/pipe_owner.py
Dual: python3 tools/rcon/chains/pipe_owner.py --node 1.21.1-neoforge
"""
...
# the pipe band x=393..394, z=20 — clear of the neighbours (P21 370/378, dye 386);
...
# Task c-foam-pipe-spray: the lock arms through the DRIED foam (the restored third
...
CHAIN = Chain(
    ...
    preferred_ports=(26108, 26118),      # this card's pinned rcon/query pair (after dye 26106/26116)
    steps=steps,
)
