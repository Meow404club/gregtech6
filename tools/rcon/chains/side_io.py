#!/usr/bin/env python3
"""machine-side-io — the two-machine face-to-face push/pull arms, live (declarative framework).

Chain semantics (task machine-side-io ACCEPTANCE): the fluid auto-IO arms of
...
command-arm on the fluid path:
  ...
  D the pull arm (:696-705): the wood barrel stacked on B (100,65,99) is FILLED
    ...
    first run). The merge key shape is loader-versioned (node_cmds, the fic
...
Run:  python3 tools/rcon/chains/side_io.py
"""
...
steps += [
    ...
    # FluidStack key shape is loader-versioned, the expect is not (the fic ruling)
