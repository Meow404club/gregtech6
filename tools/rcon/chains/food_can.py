#!/usr/bin/env python3
"""food-can-row0 — the Canner food-can row0 live acceptance chain (the
canner_refill shape: place → data-merge the two-item input → inject → the output
slot verdict).

Chain semantics (task food-can-row0 ACCEPTANCE ② — "place canner→input
...
The inventory merges ride the loader-versioned key shapes (the canner_refill
ruling): 1.20.1 `Count:1b` NBT vs 1.21.1 `count:1` component rendering. The output
expects are PER-LEG via Step.node_expects (the pattern_checker precedent): the
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/food_can.py
"""
...
# the two-item input payloads per node (the canner_refill inventory-merge ruling):
# slot 0 = the food, slot 1 = the empty can. 1.20.1 carries Count:1b NBT; 1.21.1
# renders `count:1` (the fic key-shape ruling). The cookie row merges SIX cookies.
ROTTEN_FLESH_MERGE = {
...
CHAIN = Chain(
    ...
    preferred_ports=(26110, 26120),      # this card's pinned rcon/query pair (the 2610x segment, after act 26109/26119)
    steps=steps,
)
