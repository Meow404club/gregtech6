...
CHAIN = Chain(
    ...
    Step(f'data merge block {F(_up(C, 2))} {{"gt.dungeonloot": "minecraft:blocks/stone"}}', expect="Modified",
         label="the marker (upstream gt.dungeonloot :54 — a DETERMINISTIC probe table: one stone per empty slot, 15x; SNBT compound keys carry the dot, the /data PATH grammar does not)"),
    ...
    _dump(C, 'id: "minecraft:stone"', poll=30.0, label="the LOOT rolled — a stone in this chest can only be the marker roll"),
