#!/usr/bin/env python3
"""distillation-tower — the Distillation Tower live acceptance chain (task
distill-crucible ACCEPTANCE ①: 塔 form（底 18101 + 柱 18102 + 背孔 design1）+ 1 行
分馏进出 + 背面孔输出臂).
...
task distill-output-routing (ruling 2026-09-16 distill-tower option a) the
...
rcon-chain-repair session are REBOUND onto the live run (the 7-barrel capture
census).
...
Run:  python3 tools/rcon/chains/distillation_tower.py --node 1.20.1-forge
      python3 tools/rcon/chains/distillation_tower.py --node 1.21.1-neoforge
"""
...
CHAIN = Chain(
    name="distill_crucible distillation_tower",
    slug="distillationtower",
