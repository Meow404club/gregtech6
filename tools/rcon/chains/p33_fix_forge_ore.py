#!/usr/bin/env python3
"""p33_fix_forge_ore — the forge-leg ore-invisibility root-fix live chain (task
p33-fix-forge-ore-invisible).

Byte-for-byte REUSE of the p33_ore_overlay floating-stage fixture (task
p33-ore-overlay-impl): the fix is client-render-only (GTOreBakedModel mQuads lazy
...
group: --group p33_fix_forge_ore. The fixture is idempotent (clear-then-place setblocks,
passes=1) and the band is the registered p33_ore_overlay bbox — re-staging over an
...
code): after the chain, per node,
  python3 tools/rcon/chains/p33_ore_overlay.py --world-copy --node 1.20.1-forge
  python3 tools/rcon/chains/p33_ore_overlay.py --client   --node 1.20.1-forge
