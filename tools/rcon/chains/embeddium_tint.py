#!/usr/bin/env python3
"""render-embeddium-tint — the Embeddium no-shader machine-tint live-verification chain.
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/embeddium_tint.py

The client arm (same module, no server needed after the world copy):
  python3 tools/rcon/chains/embeddium_tint.py --client baseline
  python3 tools/rcon/chains/embeddium_tint.py --client embeddium
  python3 tools/rcon/chains/embeddium_tint.py --compare a.png b.png
"""
...
def _seed_options():
    ...
    print(f"[client] options seeded: {options}")
...
def cmd_client(leg: str, wait_seconds: float = 45.0, join_only: bool = False) -> int:
    ...
    try:
        ...
        print(f"[client:{leg}] world joining...")
        time.sleep(60.0)                          # spawn + the llvmpipe chunk build
        if join_only:
            print(f"[client:{leg}] joined; leaving the game alive for manual aim")
            return 0
        ...
        for _ in range(10):
            ...
            print(f"[client:{leg}] F2 produced nothing; x11grab fallback used")
            return 0
        src = shot_dir / new[-1]
        dest = NODE_RUN / f"shot_{leg}.png"
        shutil.copyfile(src, dest)
        print(f"[client:{leg}] screenshot: {src} -> {dest}")
        return 0
    finally:
        if join_only:
            return
...
def cli(argv=None):
    parser = argparse.ArgumentParser(prog="embeddium_tint")
