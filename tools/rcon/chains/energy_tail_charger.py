#!/usr/bin/env python3
"""energy-tail-machines — the Crystal Charger live acceptance (sweep group
energy_tail; the fresh z=424 band, x520..540 — z-disjoint from the
transformer band z=420 and every roster band):

  A THE CHARGE LEG (the :970 row, T1 = V[1]=32, LU domain): the EMPTY
    energium_red_lv crystal merged into the charger's slot 0 (the P29 W4
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/energy_tail_charger.py
"""
...
def crystal_merge(pos, energy=None):
    ...
    carrier-known charge. The per-leg item carrier fork (the battery_discharge
...
CHAIN = Chain(
    name="energy_tail_charger crystal charger",
    slug="energytailcharger",
