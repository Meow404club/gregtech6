#!/usr/bin/env python3
"""c-foam-fluid-refill — the C-Foam Canner refill e2e (declarative framework, the
canner_refill shape).

Chain semantics (task c-foam-fluid-refill ACCEPTANCE 3 — "空喷剂罐+CFOAM 流体→
Canner→满罐（扩或新链，参照 cfoam_spray 链形态）"):
  ...
  B the Advanced owned refill unit (T2, dye WHITE): gt6:cfoam_owned_white 25600 (the :262
    ...
    the canner_refill T2 shape (inject 70 256 = 17920 EU, ample).

  C teardown: the explicit fill-air over the band. No global state is touched.

The 4 item-naming expects are PER-LEG via Step.node_expects (the canner_refill
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/cfoam_refill.py
"""
...
# the two canner sites on the 412 band (clear of cannerrefill's 400-408 band and
# cfoamspray's 397-398 band)
...
CHAIN = Chain(
    ...
    preferred_ports=(26111, 26121),      # this card's pinned rcon/query pair (the 2610x segment, after foodcan/cfoamspray 26110/26120)
    steps=steps,
)
