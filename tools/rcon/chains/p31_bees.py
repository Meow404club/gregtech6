#!/usr/bin/env python3
"""p31_bees — the bee-comb Lv1 static chain (task p31-bees-lv1, the p31_bees sweep
...
Two arms over the SAME honey comb input, one per consumed map:
  ...
  A Squeezer (KU pulse rig, the p29_w1_squeezer T1 form): 1 comb_honey -> 90 L
  ...
  B Centrifuge (the p29_w1_centrifuge T2 inject form): 1 comb_honey -> 100 L
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/p31_bees.py
"""
...
def feed_comb_cent(pos):
    """One comb_honey into the centrifuge's 7-slot inventory (the p29_w1_centrifuge form)."""
    return {
