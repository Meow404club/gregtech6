#!/usr/bin/env python3
"""p29-w2-eu-special/laminator — the Laminator chain (task p29-w2-eu-special
ACCEPTANCE ③+④: the HU window TIER_INPUTS row-by-row + the burning-box supply, the
p29_w1_fermenter chain form).
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/p29_w2_laminator.py
"""
...
steps = [
    ...
    Step(f"gt6machine shredder inject 1 64 {T2}", expect="progress=64/256"),
    ...
    Step(f"gt6machine shredder inject 2 256 {T3}", expect="out[0]=1x sticky_piston",
         node_expects=dict(OUT)),
    ...
    Step(f"gt6machine shredder inject 1 2048 {T4}", expect="out[0]=1x sticky_piston",
         node_expects=dict(OUT)),
    ...
    Step(f"gt6machine shredder check {FRESH}", expect="out[0]=1x sticky_piston",
         node_expects=dict(OUT), poll=120.0),
...
CHAIN = Chain(
    name="p29_w2_eu_special laminator",
    slug="p29w2laminator",
