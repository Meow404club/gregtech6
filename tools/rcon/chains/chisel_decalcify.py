#!/usr/bin/env python3
"""chisel-decalcify — the chisel item + the decalcify use-on arm acceptance chain.

Chain semantics (task chisel-decalcify ACCEPTANCE ②/④, against boiler_tank B3):
  ...
  A the give arm: the chisel lands as a REAL registered stack — chest + item replace
     ...
     landing is the headless give (the drying_rows `item replace block` precedent).
...
segment would cross-fire — the aqua-fluids lesson).

Run:  GT6_SESSION=off python3 tools/rcon/chains/chisel_decalcify.py
"""
...
# the side_io probe (x 100..120) and the fluid-gui tier row (z 200).
...
CHAIN = Chain(
    name="chisel-decalcify",
    slug="chiseldecalcify",
