#!/usr/bin/env python3
"""ulv_chain — the ULV wave RCON closure chain (task c-ulv-chain, W3).
...
probes — the GT6FeConverterCommand precedent form was NOT needed.
  ...
  B NEGATIVE: THE 8 EU PACKET IS DEAD BELOW LV (z=48): task oven-hu-conversion
    ...
    energy faces — the electricloom form): loom -> copper wire -> /gt6energy
  ...
  C NEGATIVE: THE 1375 K MELTING GATE (z=56): two wiremill_ulv rigs. Iron
    ...
    b ULV single-band ruling; 3 x 8 = 24 EU/t steady into the input faces)
...
Run:  python3 tools/rcon/chains/ulv_chain.py
      python3 tools/rcon/chains/ulv_chain.py --node 1.21.1-neoforge
"""
...
# the inventory data-merge Count key fork (the food_can precedent):
...
# the electricloom renders (the electricloom fork pattern): the input
...
# 21.1 leg namespaces the item path (the wiremill fork pattern)
