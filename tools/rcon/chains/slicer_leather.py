#!/usr/bin/env python3
"""slicer_leather — the Slicer vanilla-face live chain (task slicer-row-domain,
RCON group slicer_rows, the single slicer-leather chain per the W3 card spec):
  ...
  duration 16.
    ...
    energy: EU positive train, the canner_refill grid form — 16 EUt x 16 t = 256
...
The item-name expects are PER-LEG via Step.node_expects (the sifter
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/slicer_leather.py
"""
...
CHAIN = Chain(
    name="slicer-leather slicer_rows",
    slug="slicerleather",
