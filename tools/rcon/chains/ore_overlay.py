#!/usr/bin/env python3
"""ore_overlay — the ore OVERLAY baked-model live chain (task ore-overlay-impl).
...
placement matrix + the CLIENT-side render fixture (the embeddium_tint form):
...
server can only stage the scene, never judge pixels.
  ...
  B the ore placement matrix — the four representative cells (one per
    ...
    second-SET normal / small) with `execute if block` pins (the ore_census
  ...
  C the camera pin — setworldspawn on the platform facing yaw 0 (+Z, south), the
    wall 5 blocks south, the embeddium_tint form.
...
Run:  python3 tools/rcon/chains/ore_overlay.py [--node 1.21.1-neoforge]

The client screenshot legs (after the chain, per node):
  python3 tools/rcon/chains/ore_overlay.py --world-copy --node 1.21.1-neoforge
  python3 tools/rcon/chains/ore_overlay.py --client --node 1.21.1-neoforge
"""
...
# leaves air — the ore_census break-drop semantics; the broken BASE still rides
...
# ---------------------------------------------------------------------------
# The client half: world copy → dev-client screenshot leg (the embeddium_tint
...
def _patch_camera(dest):
    ...
    print(f"[client] camera pinned in {dats[0]}: pos={CAMERA_POS} rot(yaw,pitch)={CAMERA_ROT}")
...
def cmd_world_copy(node: str) -> int:
    ...
    print(f"[client] world copied: {src} -> {dest}")
    if any((dest / "playerdata").glob("*.dat")):
        ...
        print("[client] no playerdata in the copy — the world-spawn pin stands "
...
def _seed_options(node: str) -> None:
    ...
    print(f"[client] options seeded: {run / 'options.txt'}")
