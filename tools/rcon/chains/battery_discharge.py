#!/usr/bin/env python3
"""battery-storage (discharge leg) — the BatteryBox DISCHARGE / SUPPLY
chain (task battery-storage, RCON group battery; the same fresh
z=336 band as the charge leg, the chain-2 column x490..494):
  ...
  B THE V[i] GATE ON THE OUTPUT (x493): the same rig over an ELECTRICLOOM (task
    oven-hu-conversion swapped the oven — it books HU since its 20001-04
    rebase, so an 8 EU packet would die on the TYPE gate, not the V[i] size gate;
    the loom is the in-registry EU consumer, minIn 16, the P29 W1 form): every
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/battery_discharge.py
"""
...
# the mill feed renders fork on the item-path namespace (the P26 W1 fork pattern)
MILL_FEED = {
...
CHAIN = Chain(
    # the name carries the band key: sweep --group battery matches this member
    name="battery-discharge battery",
