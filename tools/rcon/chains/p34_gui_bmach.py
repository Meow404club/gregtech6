#!/usr/bin/env python3
"""p34-gui-basicmachine-fluids — the shared basic-machine panel fluid-seat live chain
(task p34-gui-basicmachine-fluids ACCEPTANCE ③, the p33_gui_distill_tower open-smoke
shape).
...
GTMachines.java fermenter row + GT6RecipeMaps.FERMENTER):
  ...
  A the machine places and reports its menu-less MUI-carrier form (data=-2, the
    p26_mui_row_dispatch verdict shape — the vanilla-menu byte-identical face untouched);
...
Run:  python3 tools/rcon/chains/p34_gui_bmach.py --node 1.20.1-forge
      python3 tools/rcon/chains/p34_gui_bmach.py --node 1.21.1-neoforge
"""
...
# census tops out at z=475 (the p31_fusion band 452..478) and the p33_ore_overlay
...
CHAIN = Chain(
    ...
    preferred_ports=(26652, 26662),      # this card's pinned rcon/query pair (after p33oreoverlay 26632/26642)
    steps=steps,
)
