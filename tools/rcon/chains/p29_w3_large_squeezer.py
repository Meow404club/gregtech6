#!/usr/bin/env python3
"""p29-w3-large-squeezer — the Large Squeezer chain (task p29-w3-large-12, the
p29_w3_large12 sweep group; Loader_MultiTileEntities.java:1240).
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/p29_w3_large_squeezer.py
"""
...
steps = [
    ...
    Step("data get block " + C, expect="Count: 8b", poll=90,              # the batch runs out: the 8 sticks land (one stack)
         node_expects={"1.21.1": "Count: 8"}),
    Step("data get block " + C, expect='output_tank: {FluidName: "minecraft:water"'),  # the FLUID-output leg: 800 L in the output tank
...
CHAIN = Chain(
    name="p29-w3-large-squeezer p29_w3_large12",
    slug="p29w3largesqueezer",
