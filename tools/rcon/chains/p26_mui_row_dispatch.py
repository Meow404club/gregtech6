#!/usr/bin/env python3
"""p26-mui-row-dispatch — the use() MUI dispatch-key generalization live chain.

Chain semantics (task p26-mui-row-menu-null-dispatch): GTBasicMachineBlock.use
...
the flipped arm's carrier is the documented menu-less form:
  ...
  shredder place → check data=-2    (the row-less family since the p26-mui-a-
      menu-deregistration dereg — the p26-mui-a-open-chain MUI arm, unchanged)

Run:  python3 tools/rcon/chains/p26_mui_row_dispatch.py   (add
...
# ------------------------- C: the row-less arm — the p26-mui-a-open-chain dispatch
steps += [
    phase("C: the row-less arm — the A3-deregistered shredder family: row == null has "
          "no menu column at all, the p26-mui-a-open-chain MUI dispatch unchanged "
...
CHAIN = Chain(
    name="p26-mui-row-dispatch",
    slug="p26muirowdispatch",
