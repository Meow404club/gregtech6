#!/usr/bin/env python3
"""drying-food-fluids — the food-family fluids + Drying rows live chain (declarative framework).

Chain semantics (task drying-food-fluids ACCEPTANCE — "/gt6tank fill 逐新流体臂 +
drying.py 同形 dryer round-trip，两遍 [0,0]"):

  A the four per-fluid registration arms (the aqua_fluids shape): one wood-barrel
  ...
  B the four dryer round-trips (the drying shape — dryer = TileEntityBasicMachine
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/drying_food.py
"""
...
CHAIN = Chain(
    name="drying-food-fluids",
    slug="dryingfood",
    sites=gt6world.declare_sites(*BARREL_SITES, DRYER_SITE),
    preferred_ports=(26104, 26114),      # this card's pinned rcon/query pair (P21 segment, after paintable 26103/26113)
