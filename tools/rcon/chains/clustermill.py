#!/usr/bin/env python3
"""clustermill — the Cluster Mill RU ladder live acceptance chain (task
...
driver the P26 W1 trio and the P28 arms C used).
  ...
  A THE 8 RU PACKET IS DEAD BELOW T1 (x422): one plate_iron in, an inject of 60 x 8
  ...
  B THE T2-T4 INJECT LADDER: one item in, packet = the tier max (256/1024/4096), the
    poured smoke row (plate_iron -> foil_iron, eUt 16 / duration 32) completes on every
    tier — the outputs pin 2x foil_iron.
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/clustermill.py
"""
...
# --- arm A: the 8 RU wall on a T1 (x422)
MILL_T1 = gt6world.Site(422, 64, 180)
# --- arm B: the T2-T4 inject ladder (x424..x428)
MILL_T2 = gt6world.Site(424, 64, 180)
MILL_T3 = gt6world.Site(426, 64, 180)
MILL_T4 = gt6world.Site(428, 64, 180)
...
OUT = {"1.20.1": "outputs=[2x foil_iron; ]", "1.21.1": "outputs=[2x gt6:foil_iron; ]"}
...
for tPos, tPacket, tPath in [
        (MILL_T2_P, 256, "clustermill_t2"),
        (MILL_T3_P, 1024, "clustermill_t3"),
        (MILL_T4_P, 4096, "clustermill_t4")]:
    steps += [
...
steps += [
    ...
    Step("fill 420 62 179 430 68 182 air", expect="filled"),
...
CHAIN = Chain(
    ...
    preferred_ports=(26174, 26184),      # this card's pinned rcon/query pair (the fresh 2617x segment)
    steps=steps,
)
