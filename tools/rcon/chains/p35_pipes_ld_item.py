#!/usr/bin/env python3
"""p35-long-distance-pipes — the item pipeline live acceptance (sweep group
p35_pipes_ld; the fresh z=500 band, x559..642 — z/x-disjoint from every
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/p35_pipes_ld_item.py
"""
...
steps = [
    ...
    Step(f"fill {F(WIRE_A)} 637 65 {Z} gt6:long_dist_pipe_0", expect="filled 76 blocks"),
    ...
    Step(f"data get block {F(SENDER)} gt.target", expect="gt.target: 1b"),
    Step(f"data get block {F(SENDER)} gt.target_pos", expect=f"gt.target_pos: {pl(638, 65, Z)}",
    ...
    Step(f"execute unless data block {F(SENDER)} gt.target run say LINK_CLEARED", expect="LINK_CLEARED"),
    Step(SEED_A % F(HOPPER), expect="Replaced"),
...
CHAIN = Chain(
    name="p35_pipes_ld ld-pipe-item cross-chunk item delegation",
    slug="p35pipeslditem",
