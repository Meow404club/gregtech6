#!/usr/bin/env python3
"""food-machines — the kitchen machine family live chain (task
food-machines-kitchen, RCON group food_machines):
  ...
  B the Fermenter (22003, the single-variant HU basic machine, RM.Fermenter) — the
    ...
    water through the east fill face → burning box UNDER (the fermenter
  ...
  C the Oven (20001, the RM.Furnace HU tier — the p4 machine) — a REAL row runs:
    ...
    oven-hu-conversion rebased the oven to its upstream 20001-04 type,
    NBT_ENERGY_ACCEPTED = TD.Energy.HU — the era EU rig adaptation is retired;
    the hu_steam_foundation source form stays, retyped HU) → the poll pins
    output=stonex8.

  D teardown: the explicit fill-air over the band.

passes=2 is the idempotency proof (the [0,0] of this chain).
Run:  GT6_SESSION=off python3 tools/rcon/chains/food_machines.py
"""
...
# the fresh z=340 band (east of the cracker 330 strip with margin; the west
...
steps = [
    ...
    # oven-hu-conversion: the oven books HU (its upstream 20001-04 type,
    # NBT_ENERGY_ACCEPTED = TD.Energy.HU) — the era EU rig retires with it.
...
CHAIN = Chain(
    # the name carries the band key: sweep --group food_machines matches
    name="food-machines food_machines",
    slug="foodmachines",
    sites=gt6world.declare_sites(JUICER, FER, FER_BOX, OVEN, OVEN_RIG),
    preferred_ports=(26364, 26374),      # this card's pinned rcon/query pair (after cracker 26344/26354)
    steps=steps,
)
