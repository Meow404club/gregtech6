#!/usr/bin/env python3
"""nbt_rebind_reboot_probe — the NBT_ACCESS dynamic-rebind verdict chain (task
nbtaccess-dynamic-rebind ACCEPTANCE ③, the verdict_gate RCON chain).
...
Run (this card's pinned ports 25963/25973):
  ...
  python3 tools/rcon/chains/nbt_rebind_reboot_probe.py [--node 1.21.1-neoforge]
...
# parents[3] is this worktree's root (the keepfilter_reboot_probe convention: the
...
def main():
    ...
    slug = f"rebind{'1211' if '21.1' in node else '1201'}"
    rcon_port, query_port, game_port = gt6server.pick_ports((25963, 25973, 25953))

    print(f"[rebind-probe] node {node} slug {slug}")
