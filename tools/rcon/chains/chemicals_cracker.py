#!/usr/bin/env python3
"""f1-chemicals (cracker leg) — the Steam Cracker TRUE-ROW chain (task
f1-chemicals, RCON group chemicals; the W2 smoke row is GONE — the
...
Run:  GT6_SESSION=off python3 tools/rcon/chains/chemicals_cracker.py
"""
...
steps = [
    ...
    # task cracker-machines closed the P29 gap: the catalytic feed literal is now the
    ...
    # catalytic_cracker chain owns the positive completion run), so progress=0/0
...
CHAIN = Chain(
    # the name carries the band key: sweep --group chemicals matches this member
    name="chemicals-cracker chemicals",
    slug="chemicalscracker",
