...
ops-census-mover): a literal-grep counting ledger over the port trees
...
# (registered P17, card rcon-framework-fixes) is the eight P16-card
...
SESSION_GROUPS = (
    ...
    ("hu_steam_foundation", "burning_box_family", "fluid-item-carrier",
     ...
     "steam_proof_repay"),
    ("dryer_family", "loop_closure", "boiler_tank"),
    ("boiler_distw_immunity", "large_boiler"),
    ("pattern_checker", "aqua_fluids", "side_io",
     "machine_fluid_gui", "drying_rows", "form_scaffold",
     "chisel_decalcify", "distillery"),
    # Roster backfill (card rcon-roster-backfill): the P19/P21/P23-card
    ...
    # drying and rm_backfill share the x=360 column (overlapping
    # bboxes, so one cluster's boundary cleanup covers both), and
    # drying_food sits x-adjacent with its barrel columns reaching z=59
    # (x367..381, clear of the x384 main strip below).
    ("drying", "rm_backfill", "drying_food"),
    # P26 wave1 roster expansion (card rcon-sweep-roster): the nine
    ...
    ("dye_chemical_fluids", "pipe_owner", "tag_input_machine_fallback",
     "cfoam_spray", "cfoam_blocks", "canner_refill", "act",
     "cfoam_refill", "kitchen_pot"),
    # task c-foam-block-family joins the z=20 cfoam cluster at x452..458 (clear of the
    ...
    # task rcon-roster-backfill: cfoam_refill (x410..416, overlapping the
    # act tail) and kitchen_pot (x418..426, x-disjoint from act) join after
    ...
    ("chisel_recipes", "stoneblocks_split", "paintable", "barrel_paint",
     "chisel_drops"),
    ("grass_block", "tool_hammer_wrench", "food_can"),
    ...
    # out of one wave.
    ("crucible_multiblock", "mold_faucet", "crucible_row0"),
    # Sensor band x432..436 z125..137 (single chain, roster backfill):
    # sensors_core is the roster's only session mutates member (fakesource)
    # — plan_waves gives it an exclusive wave and plan_groups splits it out of
    # a shared group (the side_io precedent).
    ("sensors_core",),
    ...
    # band.
    ("builder_wand_form_fix",),
    # P26 W1 kinetic trio (card sifter-compressor-wiremill): the three
    ...
    ("sifter", "compressor", "wiremill"),
    # P26 W1 card B (press-extruder-molds): the press + extruder ladders —
    ...
    ("press", "extruder"),
    # P29 W1 card B (kinetic-roll-ladder): the four RU roll-ladder chains —
    ...
    # admission mirroring the P26 W1 cluster form (bbox-registered, no fresh_boot /
    ...
    # outbound bridge, task cut-eu-fe-bridge.
    ("vanilla_tag_dual_tree",),
    # P28 energy-ecosystem cluster (three chains: card c-ulv-chain's band
    # + the roster backfill of card fe-inbound-chain-rewrite — the two
    ...
    # - ulv_chain: the four-arm ULV closure — arm A diesel/axle/dynamo/
    ...
    #   (margin 2 clear).
    # - fe_inbound: the FE->EU converter chain REWROTE after the
    #   EU->fe_battery bridge fell (task cut-eu-fe-bridge, the old arms
    ...
    # - builder_wand_oneclick: the one-click crucible form face, spawn-
    ...
    ("ulv_chain", "fe_inbound", "builder_wand_oneclick"),
    # P29 W1 card C (kinetic-process-ladder): the six process-family chains —
    ...
    # fakesource; the diesel rigs are per-site setblocks). --group process
    # matches through the chains' embedded name prefix.
    ("buzzsaw", "squeezer", "centrifuge", "sluice",
     "sander", "pressurewasher"),
    # P29 W1 card D (eu-hu-families): the six eu-hu family chains — one
    ...
    ("mixer_pair", "electricloom", "electricsifter",
     "boxinator", "unboxinator", "fermenter"),
    # P29 W2 card ① (energy-types-5tier): the MU/LU/CU/TU energy-type dial chain —
    ...
    # --group energy_types matches through the chain's embedded name prefix.
    ("energy_types",),
    # P29 W2 card ③ (eu-special): the Autocrafter/Lightning/Laminator chains —
    ...
    # laminator's brick burning box is a band-local setblock, the fermenter
    # firebox form). --group eu_special matches through the chains' embedded
    # name prefix (the process form).
    ("autocrafter", "lightning", "laminator"),
    # P29 W2 card ④ (exotic-energy): the six exotic-energy family chains — one
    ...
    # --group exotic matches through the chains' embedded name prefix.
    ("polarizer", "magnetic_separator", "laser_engraver",
     "laser_welder", "freezer", "cryo_mixer"),
    # P29 W2 card ② (eu-core-5tier): the five eu-core family chains —
    ...
    # electrolyzer rig is a band-local setblock). --group eu_core matches
    # through the chains' embedded name prefix.
    ("electrolyzer", "injector", "printer",
     "scanner_visuals", "slicer"),
    # P29 W2 card ⑤ (hu-tu-piggyback): the seven hu-tu family chains — one fresh
    ...
    # is band-local). --group hu_tu matches through the chains' embedded name
    ...
    ("steam_cracker", "catalytic_cracker", "coagulator",
     "generifier", "bath", "autoclave", "loom"),
    # P29 W3 card ① (nbtdesign-parts): the part-family expansion chain — one
    ...
    # --group nbtdesign_parts matches through the chain's embedded name prefix.
    ("nbtdesign_parts",),
    # P29 W3 card ② (tank-valves): the Tank Main Valve chain — a fresh z=294
    ...
    # --group tank_valves matches through the chain's embedded name prefix.
    ("tank_valves",),
    # P29 W3 card ③ (turbine-dynamo): the turbine + dynamo converter band —
    ...
    # --group turbine_dynamo matches through the chains' embedded name prefix.
    ("steam_turbine", "gas_turbine", "large_dynamo"),
    # P29 W3 card 4 (distill-crucible): the twin distillation towers + the crucible
    ...
    # and the barrel/chest sinks are band-local setblocks). --group distill_crucible
    # matches through the chains' embedded name prefix.
    ("distillation_tower", "cryo_tower", "crucible_ladder"),
    # P29 W3 card 5 (large-12): the twelve large-machine chains — one fresh
    ...
    # --group large12 matches through the chains' embedded name prefix.
    ("large_centrifuge", "large_electrolyzer", "large_coagulator",
     "large_autoclave", "large_bath", "large_mixer",
     "large_fermenter", "large_oven", "large_sluice",
     "large_crusher", "large_shredder", "large_squeezer"),
    # P29 W3 card ⑥ (heat-smelter): the three heat-family chains — one fresh
    ...
    # --group heat_smelter matches through the chains' embedded name prefix.
    ("heat_exchanger", "smelter", "melter"),
    # P29 W4 card ① (f1-chemicals): the two chemical chains — one fresh z=320
    ...
    # --group chemicals matches through the chains' embedded name prefix.
    ("chemicals_cracker", "chemicals_gas_turbine"),
    # P29 W4 card ② (battery-storage): the two battery chains — one fresh
    ...
    # --group battery matches through the chains' embedded name prefix.
    ("battery_charge", "battery_discharge"),
    # P29 W4 card ③ (eu-bridge): the three EU-bridge converter chains + the
    ...
    # only). --group eu_bridge matches through the chains' embedded name prefix.
    ("electric_heater", "electric_engine", "electric_motor", "roasting_oven"),
    # P29 W4 card ④ (hot-lube): the two hot-lube chains — one fresh z=328
    ...
    # cluster form.
    # --group hot_lube matches through the chains' embedded name prefix.
    ("hot_lube_hex", "hot_lube_lube"),
    # P29 W5 card 1 (dig-six): the six dig-tool chains — one fresh z=192
    ...
    # /gt6dig arms are band-local setblocks). --group dig_six matches through
    # the chains' embedded name prefix (the process form).
    ("dig_six_torch", "dig_six_path", "dig_six_enderchest",
     "dig_six_harvest", "dig_six_faces", "dig_six_openable"),
    # P29 W5 card 2 (blade-six): the five blade-tool chains — one fresh z=208
    ...
    # band-local setblocks). --group blade_six matches through the chains'
    # embedded name prefix (the process form).
    ("blade_six_tree", "blade_six_grass", "blade_six_rock",
     "blade_six_vine", "blade_six_faces"),
    # P31 W2 ladder cards (dig-ladder + blade-ladder, the shared GT6Tools/datagen
    ...
    # --group tool_ladder matches through the chains' embedded name prefix.
    ("tool_ladder_dig_materials", "dig_six_torch", "dig_six_path",
     "dig_six_enderchest", "dig_six_harvest", "dig_six_faces",
     "dig_six_openable", "blade_six_tree", "blade_six_grass",
     "blade_six_rock", "blade_six_vine", "blade_six_faces"),
    # P29 W5 card 4 (field-five): the four field-tool chains — one fresh
    ...
    # /gt6field arms are band-local setblocks). --group field_five matches
    # through the chains' embedded name prefix (the process form).
    ("field_five_plow3x3", "field_five_sense3x3",
     "field_five_leaf2sapling", "field_five_handdrill"),
    # task scene-six: the five scene-tool chains (ignite / sheep shear /
    ...
    ("scene_six_ignite", "scene_six_sheep", "scene_six_tripwire",
     "scene_six_scoop", "scene_six_drain"),
    # P29 W5 card 6 (electric-nineteen): the four electric chains — one fresh
    ...
    # --group electric_tools matches through the chains' embedded name prefix.
    ("electric_drill", "electric_switch", "electric_jackhammer",
     "electric_faces"),
    # P31 card (retriever-cover): the item retriever cover chain — a fresh z=248
    ...
    # --group retriever matches through the chains' embedded name prefix.
    ("retriever_cover",),
    # P31 W2 card (blade-ladder): the blade material-ladder stats chain — the
    # /gt6blade stats arms (per-material 伤害/耐久/tint verdicts through the vanilla
    # per-stack attribute map + max-damage read + the tint seam) over the tool_ladder
    ...
    # mutates member. --group tool_ladder matches the module stem.
    ("tool_ladder_stats",),
    # P31 W2 card (machine-ladder): the machine-family ladder chains — the
    ...
    # mutates member. --group tool_ladder matches the module stem.
    ("tool_ladder_machine_stats",),
    # P30 W5 card 7 (pocket-eight): the pocket multitool chains — one fresh
    ...
    # --group pocket matches through the chains' embedded name prefix.
    ("pocket_ring", "pocket_faces", "pocket_smoke"),
    # P29 W5 card t8 (armor-24, the wave tail): the three Hazmat armor
    ...
    # mutates member). --group armor matches through the chains' embedded
    # name prefix.
    ("armor_give", "armor_wear", "armor_recipe"),
    ("runtime_smoke",),   # fresh_boot singleton (decision ③)
    # P29 W5 t3 (machine-face-four): the machine-face-four chains — one fresh
    ...
    # band-local setblocks). --group machine_face matches through the chains'
    # embedded name prefix.
    ("machine_face_softhammer", "machine_face_magnglass",
     "machine_face_pincers"),
    # task trees-nine: the 9-tree worldgen chain — the sky-band
    ...
    ("trees",),
    # P30 W6 card (rocks-sticks): the surface rock trio + the stick — one
    ...
    ("rocks_sticks",),
    # P30 W6 ore registration card 1 (ore-1-mech): the registration-mechanism face —
    ...
    # mutates. --group ore_mech matches the module stem.
    ("ore_mech",),
    # P30 ore wave close-out card (ore-5-census): the live census chain — placement
    ...
    # registration trio (ledger 1 live). No mutates. --group ore_census matches
    ...
    ("ore_census",),
    # P30 W6 card (surface-blocks): the surface-plants + fallen-woods chain —
    ...
    # the natural-generation RCON gate rides the card's separate forceload+scan.
    ("surface",),
    # P30 W6 card (large-veins): the large-vein /place chain — the seed-free
    ...
    # No fresh_boot / mutates member. --group veins matches the module stem.
    ("veins",),
    # P30 W6 small-ore datagen card (small-ore-datagen): the /place feature live
    ...
    # small_ore_datagen matches the module stem.
    ("small_ore_datagen",),
    # P30 prospector micro-card (pool-prospector): the /gt6tool prospect chain — one
    ...
    # --group prospector matches through the chain's embedded name prefix.
    ("prospector",),
    # P31 card ① (implosion): the Implosion Compressor chain — one fresh z=380 band
    ...
    # no fakesource). --group implosion matches through the chain's embedded name prefix.
    ("implosion",),
    # P31 strata-lens card (strata-lens): the /place feature live chain — the lens-cell
    ...
    # --group strata matches through the chain's embedded name prefix.
    ("strata_lens",),
    # P31 card (bees-lv1): the bee-comb Lv1 static chain — one fresh z=396 band, two
    ...
    # --group bee_comb matches through the chain's embedded name prefix.
    ("bee_comb",),
    # P31 card (graagg): the Von da Graagg spawn-suppression chain — one fresh z=409..
    ...
    # arms ride summoned zombies inside the band bbox). --group graagg matches through
    # the chain's embedded name prefix.
    ("graagg",),
    # P31 card (bedrock-ore-worldgen): the bedrock-ore live chain — the chunk (4,6)
    ...
    # --group bedrock prefix-matches this key (the chain stem is bedrock_ore).
    ("bedrock_ore",),
    # P31 card (nether-lens-end-yield): the nether worldgen live chain — the chunk
    ...
    # fresh_boot / mutates member. --group nether-lens-end-yield matches through the chain's
    # embedded name prefix.
    ("nether",),
    # P31 card (fluid-spring): the bedrock-spring live chain — the chunk (4,8) band
    # (x64..79, z128..143, y-64..62; staging x288..319), the live REGISTRATION face (the
    # six spring block faces setblock+read: the five fluid-spring LiquidBlocks + the
    ...
    # chain. No fresh_boot / mutates member. --group spring matches this key exactly
    # (the chain stem is spring).
    ("spring",),
    # P31 card (massfab): the Matter Fabricator chain — one fresh z=436..446 band
    ...
    # --group massfab matches through the chain's embedded name prefix.
    ("massfab",),
    # P31 card (fusion): the Fusion Reactor chain — one fresh z=452..478 band
    ...
    # cap). --group fusion matches through the chain's embedded name prefix.
    ("fusion",),
    # P32 W1 card (usb-data): the USB Stick data-face chain — one fresh z=348
    ...
    # --group qu_usb matches through the chain's embedded name prefix.
    ("qu_usb",),
    # P32 W2 card (bees-lv2): the BumbleHive BE + worldgen chain — the fresh
    ...
    # sits x-disjoint of the scene-six band (x401..407). --group bees_hive matches
    # through the chain's embedded name prefix.
    ("bees_hive",),
    # qu-laser-domain (task qu-laser-domain): the EU->LU->EU laser chain —
    ...
    ("qu_laser",),
    # P32 W2 card (logistics-lv2): the logistics wire chain — a fresh z=248 band
    ...
    # channels). --group logistics_lv2 matches the module stem.
    ("logistics_lv2",),
    # magic-absorber (task magic-absorber): the Magic Field Absorber live
    ...
    ("magic_absorber",),
    # qu-energizer (task qu-energizer): the EU->LU->QU chain — the same fresh
    ...
    # buffers at 64/16384).
    ("qu_energizer",),
    # P32 card (placeables): the deco-TE + placed-pile chain — one fresh band
    ...
    # their pedestals per pass, the mines discard the drops). --group placeables
    # matches through the chain's embedded name prefix.
    ("placeables",),
    # P32 W3 card (logistics-lv3): the Logistics Core chain — the fresh z=236..250
    ...
    # --group logistics_core matches the module stem.
    ("logistics_core",),
    # ignition-gate (task ignition-gate): the fusion start-LU ignition gate —
    # a fresh band east of the fusion rig (x490..514, z452..478; x-disjoint from
    ...
    ("ignition",),
    # P32 W2 card (qu-scanner-replicator): the QU machine pair chain — a fresh
    # z=348 band EAST of the qu_usb chest column (the rigs x458/x466, band
    ...
    # the massfab-chain form restored in teardown). --group qu_machines matches
    # the module stem.
    ("qu_machines",),
    # P33 W1 card (circuits-parts): the circuits-part registration chain — a
    ...
    # pinned offline by GT6CircuitPartsRegistrationTest), north of the parts
    ...
    # --group circuits_parts matches through the chain's embedded name prefix.
    ("circuits_parts",),
    # logistics-covers-12 (task logistics-covers-12): the logistics cover family
    # live acceptance — a fresh band SOUTH of the logistics_core band (z252..264,
    ...
    # --group logistics_covers matches the module stem.
    ("logistics_covers",),
    # P33 W2 card (circuits-crafting-c): the circuits C-column crafting chain — a
    # fresh z=290 band (the chest+ACT column pair on x420/x422, between the
    # circuits_parts z=284 and the parts z=292 with margin). Admission
    # mirroring the circuits_parts form (bbox-registered, no fresh_boot /
    ...
    # --group circuits_craft matches through the chain's embedded name prefix.
    ("circuits_craft",),
    # P33 card (cracker-machines): the cracker verification chain — the Steam +
    # Catalytic Cracker live positive runs on the TRUE rows (steam+propane -> the
    # four-gas ladder / H2+ethanol+Pt -> ethylene+propylene, the P29 W2 chains'
    # proven forms) over a fresh z=330 band (x/z-disjoint from the P29 W4 chemicals
    ...
    # GT6CrackerCraftingJsonTest). --group cracker_machines matches through the
    # chain's embedded name prefix.
    ("cracker_machines",),
    # P33 W2 card (gui-distill-tower): the distillation-tower GUI chain — a
    # fresh z=356 strip east of the t7 pocket band (z=352 is taken by the
    ...
    # --group gui_distill matches through the chain's embedded name prefix.
    ("gui_distill_tower",),
    # food-fluids-b1 (task food-fluids-b1): the food-fluid batch-1 live
    ...
    # mirroring the dyechemical form (bbox-registered, no fresh_boot / mutates
    # member — the tank arms are the /gt6tank command channel).
    # --group food_fluids_b1 matches through the chain's embedded name prefix.
    ("food_fluids_b1",),
    # food-machines-kitchen (task food-machines-kitchen): the kitchen machine
    ...
    # burning-box train (the fermenter supply form) and the Oven 20001
    # RM.Furnace row, all in the fresh z=340 band (east of the cracker 330 strip).
    # --group food_machines matches through the chain's embedded name prefix.
    ("food_machines",),
    # P33 card (ore-overlay-impl): the ore OVERLAY baked-model live chain — a
    ...
    # plus the enclosing 3-high walls and the camera pin — the embeddium_tint
    ...
    # ore row is the client fixture, passes=1 persists it). --group ore_overlay
    # matches the module stem.
    ("ore_overlay",),
    # P33 card (fix-forge-ore-invisible): the forge-leg ore-invisibility root-fix
    # live chain — BYTE-FOR-BYTE reuse of the ore_overlay floating stage (same
    ...
    # discipline drying/rm_backfill established).
    # --group fix_forge_ore matches the module stem.
    ("fix_forge_ore",),
    # food-fluids-b2 (task food-fluids-b2): the drink-seam live chain — a fresh
    # barrel band x=404 z=20 (x-adjacent-but-disjoint from foodb1's bbox 397..399, the
    ...
    # representatives pinned at 250 mB stat). Admission mirroring the foodb1 form
    # (bbox-registered, no fresh_boot / mutates member — the tank/drink arms are the
    # /gt6tank + /gt6drink command channels).
    # --group food_drink matches through the chain's embedded name prefix.
    ("food_drink",),
    # food-tail (task food-tail): the card-block-outside drink-fluid live
    # chain — a fresh barrel band x=420 z=20 (bbox 418..422, x-disjoint from
    # fooddrink's 402..406 and the act/cfoam 408..416 run), the metal-barrel
    ...
    # census. Admission mirroring the fooddrink form (bbox-registered, no
    # fresh_boot / mutates member — the tank/drink arms are the /gt6tank + /gt6drink
    # command channels). --group food_tail matches through the chain's embedded
    # name prefix.
    ("food_tail",),
    # bees-lv3-a-items (task bees-lv3-a-items): the bumblebee item-domain live
    # chain — a fresh chest band x=384 z=360 (z-disjoint from the qu_usb band z=348
    # and the gui_distill tower band z=356), the 8-face princess/drone/queen/dead
    # registration census (the qu_usb chest form) + the gt.bumble 13-key gene
    # compound + the port species-code int (gt.bumble.meta) carrier arms with the
    # code-overwrite proof. Admission mirroring the quusb form (bbox-registered,
    # stateless placement, no fresh_boot / mutates member).
    # --group bees_a matches through the chain's embedded name prefix.
    ("bees_a",),
    # bees-lv3-b-bumbliary (task bees-lv3-b-bumbliary): the Bumbliary breeding
    # live chain — a fresh band x=390 z=368 (z-disjoint from the bees_a band z=360,
    # the gui_distill band z=356 and the qu_usb band z=348), the placement +
    ...
    # the bees_a form (bbox-registered, no fresh_boot / mutates member).
    # --group bees_b matches through the chain's embedded name prefix.
    ("bees_b",),
    # bees-lv3-c-hive-loot (task bees-lv3-c-hive-loot): the hive LOOT live
    ...
    # fill-count + bit-identical recompute arm (the bees_hive arm-C form, now
    ...
    # Admission mirroring the bees_b form (bbox-registered, no fresh_boot /
    # mutates member).
    # --group bees_c matches through the chain's embedded name prefix.
    ("bees_c",),
    # gui-basicmachine-fluids (task gui-basicmachine-fluids): the shared
    # basic-machine panel fluid-seat live chain — a fresh z=484 band (the roster census
    # tops out at z=475, the ore_overlay stage starts at z=508), the Fermenter 22003
    ...
    # mirroring the gui_distill_tower form (bbox-registered, no fresh_boot /
    ...
    # baselines). Admission mirroring the sensors_core form (bbox-registered,
    # no fresh_boot / mutates member).
    # --group sensors_trivial matches through the stem substring.
    ("sensors_trivial",),
    # bumbliary-gui (task bumbliary-gui): the Bumbliary GUI acceptance chain —
    # a fresh z=376 band (z-disjoint from the bees_b band z=368 and every registered
    ...
    # Admission mirroring the bees_b form (bbox-registered, no fresh_boot /
    # mutates member).
    # --group bumbliary_gui matches through the chain's embedded name prefix.
    ("bumbliary_gui",),
    # bumbliary-recipes (task bumbliary-recipes): the Bumbliary pair crafting +
    # hive-carry live chain — a fresh band x-disjoint from the bees_c band 479..520
    ...
    # carrying the BOX + the contents (the bees_c arm form, now with the
    # TileEntityBase04MultiTileEntities.java:166-171 block-item face). Admission
    # mirroring the bees_c form (bbox-registered, no fresh_boot / mutates member —
    # the rigs are band-local setblocks).
    # --group bee_recipes matches through the chain's embedded name prefix.
    ("bee_recipes",),
    # loot-injection (task loot-injection): the dungeon-loot injection live
    ...
    # ForgeHooks.modifyLoot path). Admission mirroring the sensors form
    # (bbox-registered, no fresh_boot / mutates member).
    # --group loot_inject matches through the chain's embedded name prefix.
    ("loot_inject",),
    (
        # covers-gameplay-10 — the gameplay cover family band z=300..312
        ...
        "covers_g2",
        # covers-display-scale-6 — the display/scale cover family live
    ...
    # machines-bumblelyzer-crucible (task machines-bumblelyzer-crucible): the
    # Bumblelyzer scan arm + the Crystallisation Crucible :683 boule row live chain —
    # a fresh band z=384 (z-disjoint from the bumbliary_gui band z=376), the
    ...
    # the rigs are band-local setblocks/gt6machine places (the cracker_machines
    # admission form, no fresh_boot / mutates member).
    # --group machines_bc matches through the chain's embedded name prefix.
    # boule-consumption-rcon joins the cluster (task boule-consumption-rcon):
    ...
    ("machines_bc", "boule_crusher_gem4", "boule_lathe_sticklong3"),
    # machines-bp (task machines-burner-plantalyzer): the Burner Mixer +
    ...
    # food_machines form (bbox-registered, no fresh_boot / mutates member).
    # --group machines_bp matches through the chain's embedded name prefix.
    ("machines_bp",),
    # pool-cover-hosts (task pool-cover-hosts): the real cover-host live
    ...
    # + the drum gate negatives. Admission mirroring the covers_g2 form
    # (bbox-registered, no fresh_boot / mutates member — the rigs are band-local
    # setblocks). --group cover_hosts matches through the chain's embedded
    # name prefix.
    ("cover_hosts",),
    # P35 card (energy-tail-machines): the energy-tail two-chain group — a fresh
    ...
    # energy_tail matches through the chains' embedded name prefix.
    # task energy-zpm-dechargers: the RESERVED energy_tail_zpm seat is CLAIMED —
    # the third chain (energy_tail_zpm) rides the ZPM item + the two decharger
    # BEs (a fresh x544..558 z420..424 column, x-disjoint from the P35 band columns).
    ("energy_tail_transformer", "energy_tail_charger", "energy_tail_zpm"),
    # rails-31-blocks (task rails-31-blocks): the three rail chains — a fresh
    ...
    # race is tick-count-locked, wall-clock independent). --group rails matches
    # through the chains' embedded name prefix.
    ("rails_ride", "rails_booster", "rails_detector"),
    # portals-mini-nether-end (task portals-mini-nether-end): the two
    # miniature-portal live chains — the nether chain z=358 band (x446..451,
    # z-disjoint from the machines_bp band z=350) + the end chain z=384 band
    # (x509..514, disjoint from the machines_bc z=384 band in x — the bc band
    ...
    # --group portals matches through both chains' embedded name suffix
    ...
    ("portal_nether", "portal_end"),
    # slicer-row-domain (task slicer-row-domain): the Slicer vanilla-face live
    ...
    # mirroring the sifter form (bbox-registered, no fakesource, no mutates —
    # the grid inject rig).
    # --group slicer_rows matches through the chain's embedded name prefix.
    # Cluster member = the chain FILE stem (select_groups load_chain contract,
    # the machines_bc multi-stem form) — the group key itself resolves via
    # Chain.name "slicer-leather slicer_rows".
    ("slicer_leather",),
    # long-distance-pipes (task long-distance-pipes): the LD pipeline two-chain
    ...
    # setblocks/fills (the energy_tail admission form, no fresh_boot / mutates
    # member). --group pipes_ld matches through the chains' embedded name prefix.
    ("pipes_ld_item", "pipes_ld_fluid"),
    # chains-ld-transformer (task chains-ld-transformer): the Long Distance
    ...
    # band-local setblocks). --group energy_ld matches through the chain's
    # embedded name prefix.
    ("energy_ld_transformer",),
    # P37 chains card (chains-water-wheel): the water-wheel RU-source live
    ...
    # setblock fence). --group kinetics_water_wheel matches the module stem.
    ("kinetics_water_wheel",),

    # P37 pool card (chains-coke-oven-run): the coke-oven LIVE RUN chain — a
    ...
    # chest (the two GT6LaserGas registrations read back, the qu_usb
    ...
    # drives the run, no rig needed). --group coke_oven_run matches the
    # chain's embedded name prefix.
    ("coke_oven_run",),

    # chains-anvil (task chains-anvil, the P37-d s4-2 gap): the anvil
...
# --- sweep session lock (P32, card ops-sweep-lock) ------------------------
#
...
# fold_units_defs is the one pinned invariant (units-convergence terminal
...
CENSUS_PATTERNS = (
    ...
    ("api_consumer_MT_files", r"^import gregapi\.data\.MT;", "files",
     "research.api-modernization strangler census (76 files @ 2026-09-17)"),
    ("api_consumer_TD_files", r"^import gregapi\.data\.TD;", "files",
     "research.api-modernization strangler census (108 files @ 2026-09-17)"),
    ("api_consumer_OP_files", r"^import gregapi\.data\.OP;", "files",
     "research.api-modernization strangler census (51 files @ 2026-09-17)"),
    ("api_consumer_CS_files", r"^import gregapi\.data\.CS;", "files",
     "research.api-modernization strangler census (18 files @ 2026-09-17)"),
    ("fold_units_defs", r"static long units\(", "lines",
     "units-convergence terminal state == 1 (UT.java:39); the Converter:221 lesson"),
)
