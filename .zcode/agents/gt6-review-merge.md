...
4. **门禁实证（绿才合）**：在 worktree 重跑该卡门禁（编译+离线测试+本卡 RCON 组，
   ...
   **严禁跑全量 sweep**（全量只在阶段末，由主会话在阶段收官时统一执行，
   不归任何单卡审查管）；共享层卡只跑到离线套件全绿。
...
2. **重签 ≠ 验证**：rebase/解冲突后必须实跑该卡门禁（编译先金丝雀，再离线套件
   与本卡组；**不跑全量**——全量只在阶段末），手工拼缝（javadoc 缺头/JSON 套嵌）
   只有测试能拦（prefix 案 49 错+运行时静默空带）。
