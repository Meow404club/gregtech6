...
`第 17 阶段：21.1 机器 capability/IO 面收口 + datagen 共享树裁决 + RCON 框架五小修`（**2026-09-06 收官**：4 卡全 approve 零打回合入 main HEAD 3dee46ff——datagen-node-local-output a2d0d7fa（ADR-P17-1 输出目录按节点参数化：正典节点共享 tracked 树不变、21.1 输出节点本地 build/，五步门禁取代六步，21.1 二跑 written:0 首次可达，datagen_tree_check.py 双树 byte 断言）/ rcon-framework-fixes 6a0958a2（chain.node 回写+session slug 泛化+SESSION_PORTS 尊重 preferred_ports+sweep p16 八链簇+boot 归属门，selftest 24 检）/ 2111-test-fixture-22red 6e1d0d55（19 红转绿：罐头 JSON 21.1 对象形双副本+CoverClient standalone 键取+MaterialBlocks leg-conditional+FluidBridge map-miss）/ 2111-machine-io 3dee46ff（GT6CapabilityWiring 补 DRYER_BE/DISTILLERY_BE 双面注册+SeamTest 9→11+家族守卫常量表+census 测试——两 RCON delta 单根因=漏出 P15 wiring 注册面，修复 21.1 实证转绿）；终局门禁：root 205+mdk 1074=**1279 全绿**，1.21.1 失败集 **IDENTICAL-22→3**（残集 Barrel permanent+EngineFuels/OreChain defer 三元组逐条等值），1.21.1 compile 0 红，RCON 1.20.1 27/27+1.21.1 27/27 推定全绿（25 绿基线+两 delta 链双遍实证），runData 五步门禁 1.20.1 二跑 written:0；docs 镜像=ADR `2026-09-05-p17-datagen-tree.md` + `2026-09-05-p17-machine-io-adjudication.md`）
