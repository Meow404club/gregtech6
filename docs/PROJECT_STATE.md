...
`第 19 阶段：移植批（凿石宇宙 / DRYING 第二批 / distillery front 正典化 / chisel 配方+右键门）+ B' 动态注册表重绑 + RCON 框架小修`（**2026-09-06 收官**：7 卡全 approve 零打回，三审查会话 3/3+3/3+1/1，main HEAD 01031e47——stoneblocks-registry 2948b7f9（GTStoneBlocks 独立注册面：17 石×16 变体 EnumProperty+四映射表 byte[] 直译+oreDictMappings；否决 GTMaterialBlocks 扩展）∥ drying-rows-backfill-2 a5278570（盐 2 行+矿物脱水 8 行+Clay 循环 6 成员+BlockDiggable:73+SKIPPED 8 条审计；seawater/waterdirty 走 SIMPLE_LIQUID_SPECS 否决 AQUA_SPECS 追加；water_hot 裁不注册=IC2 别名 parity）∥ distillery-front-canonical 6094b50b（三张 front colored+overlay src-over bake 正典化，幂等脚本+四源三产物 sha256 全录；overlay_active 实为 16x64 四帧 strip 纠偏研究卡）+9bc08d3c runData 收口（272 石键 lang 入 generated）∥ stoneblocks-render 8acc97f6（272 PNG byte-identical 借入+17 blockstate×16 变体+272 cube_all 模型+stoneLoot 自掉表子 provider；loot 坍缩/item 单 parent/不去重三偏离 javadoc 钉住）∥ chisel-recipes 01031e47（CHISEL 图 RM:138+三源 36 行 pour+ToolCompat:224-229 右键门付 10000+gt6:variant tag 双腿 port-ism+GT6ChiselCommand 越卡偏离 ACCEPT 三要件）∥ rcon-waitdone-8kb a374033e（8KB 尾窗盲态→字节偏移增量扫描+error_tail 归因，selftest 39→48 检）∥ nbtaccess-dynamic-rebind b2e16830（**B'**：NBT_ACCESS static final AtomicReference 委托+AboutToStart 抓 composite，frozen builtin 兜底；1.21 一切附魔数据驱动下 frozen 视图 parse 静默剥附魔——终态按实测修正研究卡预测；RCON 双腿 ENCHANTMENT SURVIVED）；裁定三单：W4 签名波不做条件池化（全树 21.1 NBT 面 level 依赖=0+saveToItem 0 调用）/circuit 配方行整面裁池随工具卡/上游无编程器纠偏；终局门禁：root 205+mdk **1132**=**1337 全绿**，1.21.1 compile 0 红+test **1134/0/2skip=IDENTICAL-0 保持**，runData（render 波）二跑 written:0+tree_check byte-identical，RCON 新三链实弹 [0,0]；docs 镜像=ADR 两篇 `2026-09-06-p19-{w4-nbt-provider-ruling,porting-split}`）
