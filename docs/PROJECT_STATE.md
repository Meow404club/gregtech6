...
`第 25 阶段：tag 双向范式②配方输入（机器面 fallback）/ C-Foam 管泡沫最小面 / 锤扳手工具+食品罐 row0 / 漆回流点键修复 / 测试卫生 / datagen 双树值形归一 / KubeJS 适配裁定 / 四向调研`（**2026-09-08 收官**：7 实现卡全 approve 合入（审查会话 S1 5/5+S2 3/3 两轮换零悬案，追加式先派后补；统一合并序=hammer→c-foam/food-can→tag-fallback 押后 rebase 重签重验最后合），main 实现链 b17a7a66→71bd4286+docs 297d1d20（全景普查 32 条镜像 TODO）/dab8d735+1a0c9ca3（AGENTS.md KubeJS 条款+SPEC 模板句）/4180e7ae（ADR 三篇正典）——【用户新裁定】KubeJS 魔改适配常设：每张移植卡 SPEC 必须声明 KJS 适配面（模板句入宪法）；tier-a=datapack 原生（现状零成本）/tier-b=RM 配方图 JSON 直灌（P26 候选）/tier-c=GTCEu 式 kjs 绑定（按需，1.21 腿优先=KubeJS 7 json schema+implements 先例；modCompileOnly 零传染）【实现七卡】①p25-tag-input-machine-fallback：Recipe.checkStacksEqual 两段式（精确短路+matchesByMaterialTag 三门）+**方向铁律**（TagKey 只从配方输入侧派生测机器输入）+sTagTest 谓词注入缝+itemTagFamily 上提共享——上游 unification 语义的现代原语化，tag 双向范式第②向闭合（ADR 正典）②p25-c-foam-pipe-spray：喷剂 32+applyFoam 写点+allowInteraction 第三条款回填（!mFoamDried）+干燥 rng 注入缝+渲染快照 fresh 叠加/dried 换装+NBT 回流三键（owner 不回流，onPlaced 记新放置者）+/gt6pipe spray|dry|removefoam——21.1 腿链 [3,3]=显式声明偏离（copy_nbt 运行时改名不解析，GTPipeFoamTest 11/0 双腿兜底，演进路线=按腿生成+归一器）③p25-tool-hammer-wrench：锤/扳手 GT6FileItem 同形+craftRemaining 静态缝复用（has/get 成对）+ToolAction×2+tag×2+三合成行（石头路线输入 #minecraft:stone_tool_materials 双源实证）④p25-food-can-row0：GT6FoodCans 8 meta 最小子集（44 meta 冻结）+弯管最小物品+RM.food_can row0 三行（EUt16/16t 分档 switch 逐字）⑤p25-paint-loot-dotkey-fix：**P22 既有缺陷修复**（copy_nbt 点键未引号被 NbtPath 切分→gt.color/gt.painted 回流自 P22 从未生效+伪造空 gt 复合；修=引号形+26 表重生成+语义探针+负例钉缺陷；与 P23 读侧修复接力才全通）⑥p25-chisel-test-resolver-hygiene：sStoneItemResolver 污染链修复，类序彩票消灭（solo 11/2→11/0+9/0）⑦p25-datagen-tree-check-unify：VALUE_NORMALIZERS 两层归一（SEGMENT_MAP 不动+三带九变换方向恒 node→canonical；228 恒红归零 exit 0+负例自证不吞差；ADR 正典）【审查质量】S1 补跑 hammer 卡 21.1 腿门禁缺口（1411/0 补齐）+裁决五处夹具双过滤偏离（严格强化）+实解 ZH_KEY_FLOOR 三方棘轮×2（2619/2629）+c-foam quoted() 漏网冲突实解；S2 发现 id329 裸键第五次闪现（自愈+平键重写）【四向调研（用户令乘并发低）】未移植全景普查 32 条新池（上游 15 节 vs 本仓 13 注册类 9 域差集：坩埚链=GT6 开局第一优先/基础机缺 46/53/管道材质阶梯/储罐主阀 26 台/EU 桥/世界生成域）；GTCEu 表现层三态目录（工具图标=GregTech Refreshed MIT 可借+灰度 tint 陷阱；材质图标/机器贴图不借已覆盖）；mod 适配面（Embeddium/Iris 冒烟 P26 首选零适配；EMI/REI 缓→随首个真 JEI 类目捆绑；Create/AE2/MEK capability 天然兼容；TOP/地图不做）；ModularUI 复用面（9 vanilla Menu+ACT 1 MUI；迁移批次 A shredder/crusher/lathe panel 工厂→B 流体面→C oven；widget 库全在 vendored 树）【终局门禁】RCON 全集：sweep 27 链双腿全绿（forge wall 2022s/neo 2006s，failures none）+九链双腿 17/18（唯一红=p25cfoam neo [3,3]=声明偏离）；各卡 cleanTest 递进 1392→1409→1411→1420→1430→1443→1454（forge）/1394→1456（neo）；治理=ADR 三篇+known_bugs 双更（state_update 双重编码 merge=false 也中招=工具缺陷登池/paint-loot 点键已修待关账）+否定性裁定两条防重复普查（树叶原木 tag 无对象搁置/size-seam keep-deferred）+新建 p25-neo-node 双腿 worktree）
