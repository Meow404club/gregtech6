...
`第 23 阶段：jar 正典名 / ModularUI 独立子仓库 / 材料汉化（根修+442 回填）/ 桶漆域三件套 / 喷漆罐实体腿 / Jade universal 流体升级 / RCON 五债 poll 化+node_expects`（**2026-09-07 收官**：11 实现卡 12 审 approve（S1 4+1 reject 打回重交 S3——裸名 stonex8⊂cobblestonex8 假绿被审查拦下；全程合并串行持牌），main HEAD 0f26c825——【用户三指令】jar 正典名 gt6-<节点>-<mod_version>.jar（32cb832d archivesName knob+jar_content_check/jarjar_smoke glob 优先防旧名字母序遮蔽）∥ ModularUI 独立子仓库（66ba69db 案A：git submodule 按原路径挂回+git subtree split 全史迁移 MGT6GA-repos/modularui.git 9 提交；接线真零改动=include/jarJar 消费/jar check 三处全路径型；案B/C 死点=MDG jarJar 仅对 ProjectDependency 注入 SRG 变体 LegacyForgeModDevPlugin.java:214-229；运营纪律=worktree 逐个 protocol.file.allow init+deinit 连坐共享 config+191MB 残留挡 clone 入 ops.discipline）∥ 材料汉化双卡（根因=MaterialPrefixItem:65/GTMaterialPrefixBlockItem:57 把 mNameLocal 纯英文字面量当 %s 填参→1769 个 gt6.material.* zh 键运行时零消费显「Bronze锭」；C1 76946849 materialFill 共享缝嵌套 translatable（slug 与 en walk 同函数+60026 注册对裸键哨兵）；C2 0528b1b8 回填 442 键（tagprefix 365=identity 213+dump 投 57+手译 95/block 28/fluid 21/spray 21/material 5/tab 2；zh=en=2531 集合相等零翻译债；dump 主投+TeamNED 复投，36 行社区复核清单在档））【桶漆域三卡】（architect 重定性：桶 08 基类随 03 族已 implements IPaintableTE——NBT 层本就能漆，真缺口=物品携漆/tint 消费/跨缝 bug）7200f5a0 物品携漆根键对（forge 根 tag/neo CUSTOM_DATA 信封+load 门 round-trip 白漆保旗裁定上游 :85）→44bc6d81 tint 消费面（16 块 tinted 单 element+Block/Item 双注册；GTMachinePaintTint/GTItemPaintTint 类文件 diff 空硬约束；runData 双腿二跑 written:0 跨腿零漂移）∥ 6e617047 掉落已漆机器不显色 bug（loot 写 BlockEntityTag.gt.* vs 读根 tag 两缝不相交；读侧两级 BlockEntityTag 旗门控+根回退，写侧零改）【实体腿】3d502bac 羊（未剪毛）/狼（已驯服）染色 DyeColor.byId(~mColor&15) 恒等零查表+实体击 -50/次+同色不耗+除漆剂无实体臂=上游即死；21.1 Wolf.setCollarColor private 惰性反射桥=审查裁定正解（NeoForge patch 未放宽+无 AT/mixin 基建+fail-visible）【Jade universal】6d431bb4 v1 pair 退役→registerFluidStorage 正字标（GTCEu GTFluidStorageProvider 姿势单体双腿同体；priority BODY-1 双腿 javap 实锤抢跑 Jade 自家 capability Extension 否则 long 回退 int 钳位；FluidTextHelper 双腿语义分叉 4,000mB vs 缩写按腿钉死）【RCON 五债】四链单发 poll 化（等终态替代 sleep 节奏依赖；ADAPTIVE_QUIET on/off 对照实锤 P18 quiet 窗砍 10 倍时序预算+断言步 sleep 挂 judge 后从未生效=债真形态）+p16pchk s17 整行双钉 node_expects 双腿分叉（框架新增判定期按 chain.node 解析；import 期分叉 sweep 21.1 腿会错腿 sweep.py:156-157 坐实；selftest 48→51 检含错腿红反 no-op；README 正典化+裸名单 token 禁令）【终局门禁】S2 收官证书 forge cleanTest **1242/0/0/0**（=1217+5+8+3+4+2 逐卡吻合）；RCON 五链双腿 [0,0]（s17 poll 4 次 not yet 后才绿=绿不早于烧完）；jar 双腿重编 gt6-1.20.1-forge-0.1.0.jar 27.7MB/gt6-1.21.1-neoforge-0.1.0.jar 27.4MB+jar_content_check GREEN 40 PASS；known_bugs 关四条（RCON 三条+tag-mismatch）；ADR 两篇正典（modularui-subrepo/barrel-paint-domain））
