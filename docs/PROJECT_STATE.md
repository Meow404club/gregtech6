...
`第 21 阶段：Jade 兼容 / 机器喷涂 Paintable 两卡 / 整备批（i18n 缝归一·石块 272 拆分·ModularUI headless 冒烟）/ drying 食物族 / 凿石掉落转换`（**2026-09-07 收官**：8 实现卡全 approve 零打回，main HEAD 5660874e，审查节奏 1｜1+1 追加｜1+1+1 追加×2——【研究三张】paintable（TileEntityBase07Paintable 存储 mRGBa+mIsPainted/NBT gt.color+gt.painted/反码复合恒等可直存/双腿唯一分叉=ModelData 宿主接口名；范围=07 全族 26+ 类）∥ jade（双腿 harvest tmp/harvest/jade-{1201,1211}+无官方 maven 走 Modrinth+GTCEu GTJadePlugin 参照+上游 1.7.10 无 WAILA 面+curator partial 入库 1569 文件）∥ drying 三主体（食物族 4 行可活/树脂族与生物族上游即死不开卡/GTCEu 无 Dryer=本移植首个现代翻译）【Jade】a7da6593：gregtech6.jade 新包零静态引用隔离（NCDFE 防线实证）+GT6MachineProvider 四段 tooltip（进度着色/能量+输入带 RU·KU/成形态/错误行）+compileOnly 双腿钉值 11.13.3+forge/15.10.6+neoforge+编译实证三分叉补录（BoxStyle 互缺/ITooltip.getElementHelper 删/IJadeProvider.getUid 抽象）【喷漆】7a19b191+f910bde8：IPaintableTE 挂 TileEntityBase03 全族+存储直存免反码+通道平均混色（UT:1576 四钉值复核）+RCON /gt6machine paint|unpaint 臂+GTMachinePaintTint 21 块 63 模型 per-face tintindex 0（未漆=白乘恒等 -1 哨兵零回归实证；colored/ 灰轨 repo 无=显色度偏差池）【整备批】cbe37e2e（materialWalkEmittedKeys Set→不可变 Map，2-copy 消除+byte-identical 三点核验）∥ 0e3f8524（石块 17×1 property→272 per-pair+272 loot 独立表，variant0 保裸 snake id，四映射表 byte[16] 原样）∥ 40949bdc（ModularUI 离线 headless 冒烟：test sourceSet 双腿 18+18，unit.testing=true 上游钩子+Bootstrap 双闸，cleanTest --rerun 破缓存取证；"2M"=0.002 EvalEx 大小写哨兵）【drying 食物族】a623e78d：4 流体 fluid-only（sap/maplesap/reedwater/cactuswater，FL null-texture=vanilla water+tint 零借图）+4 行逐字浇回（EUt16；live "39 loaded, 3 skipped" 与上游 :530 守卫保真）【凿石掉落】5660874e：170 表 alternatives[match_tool(gt6:chisel)→CHISEL_MAPPINGS]+102 直通（上游臂③ 零额外耐久/无视 silk·fortune 语义 loot 化保真；21.1 items 序列化 delta=p17 单数化口径）【终局门禁】合并态 forge cleanTest **1187/0**；分发 jar 双腿重编+jar_content_check GREEN；RCON 新链三条 p21_paintable/p21_drying_food/p21_chisel_drops 双遍 [0,0]；ADR 五篇正典（smoke-gui/stoneblocks/paintable/jade/chisel-drops））
