...
`第 22 阶段：喷漆显色度修正+工具物品域 / Jade 流体显示段 / 整备批（boiler form 拒绝臂·stonecutter 正则劈分）/ oven 双腿复验实测`（**2026-09-07 收官**：7 卡全 approve 零打回，main HEAD b9b0b23f，审查节奏 1+1+1 追加｜1+1+1 追加——【研究三张】paint-grayscale（**翻案**：colored/ 目录 repo 内已在且本身即灰度待染轨、P20 已 byte-identical 借入，机身显色与上游零差——P21「灰轨 repo 无」结论推翻；真实偏差唯一=front 状态 overlay 被 tintindex0 连带染，C1 偏差声明落 known_bugs painted-machine-front-overlay-tint）∥ jade-fluid（数据缝=机器罐同步触发面不含流体必须 Jade 服务端推；复核修正 C-1 多块类零罐字段单分支/C-2 FluidTankGT amount() 上游 :330 同形已存在零触碰）∥ spraycan-toolitem（上游 Behavior_Spray_Color 16 色罐+除漆剂与本仓 IPaintableTE 完全对位；circuit crafting 判定不同车留池）【喷漆域三卡】96f9d355 front overlay 拆层（双 element：body 灰轨 tintindex0+decal 无 tintindex 键+epsilon 外浮 cullface north 防 z-fight；24 PNG 借图 sha256 台账+63 模型重生成 runData 二跑 written:0）∥ 7ba12b14 painted-item-domain（21 表 copy_nbt 携漆 block_entity→BlockEntityTag 恰 2 REPLACE op+GTItemPaintTint 显式注册（vanilla ItemColors:25-93 无 BlockItem 委托铁证）+**13 块 loot 表缺失既有缺口修复**（oven+三梯此前破坏零掉落）；21.1 分叉 CopyCustomDataFunction）∥ cc21dd96 spraycan-items（16 色罐 512 用+除漆剂 256 用+空罐 swap 目标；gt.remaining LONG ×10 单位制；paintPaintableTE 路由 04:227-235 逐字；原版白名单 DyeColor.byId(15−i) 折叠；18 PNG 借图台账；三偏离=隐形罐裁/无目标臂池/FIRE_EXTINGUISH 占位音效）【Jade 流体段】b9b0b23f GT6FluidProvider（单分支 instanceof BasicMachine 读 mTanksInput/Output 写 GT6FluidsIn/Out 自描述键 {FluidName, Amount long, Capacity long}；客户端 of(fluid,1000) 图元+真 long 文本避 readDefault INT_MAX 坑；v1 内嵌形态零新分叉，universal registerFluidStorage 入池无沉没成本；runServer 双腿冒烟 ERROR=0）【整备批】3ae605aa boiler form 拒绝臂（declaration-only 硬失败分类测试化+reason 串逐字零迁就+creative probe 撞 Forge FluidType.SIZE 注册表墙按预案降级裁池留痕——离线勿 probe new Player/Entity 系教训）∥ f58fb0a5 stonecutter 正则劈分（.color( 按实参形态两路：4 参逗号形 lookahead 留表+单参删表改 //? 行内分叉；21.1 生成源码复核 4 参 8 处全命中/单参残留 0/provider:127 归一活体证明）【oven 双腿复验】sweep --dual 27 链全集实测升格 P17 推定（forge 23/27+21.1 22/27；4 链确定性时序红=既有时序债立 known_bugs rcon-single-shot-timing-reds-diesel-boiler-loop 头号嫌疑 P18 quiet 节奏从未全集复验+p16pchk 链 expect 串 namespace drift 格式债；oven 序列双腿全绿 stonex8 命中）【终局门禁】合并态 forge 全量 **1217/0**+neo **1219/0/2skip**（cleanTest 强制读 XML 实数）；分发 jar 双腿重编 27.7/27.4MB+jar_content_check GREEN；RCON oven 序列+--dual 全集落账；ADR 四篇正典（front-overlay-split/jade-fluid-seam/spraycan-toolitem/stonecutter-regex-split））
