...
`第 20 阶段：视觉 / i18n / GUI 框架三线 sprint（贴图上游填充 / 标准 i18n zh_cn+拼接化 / ModularUI fork+JarJar）+ 移植进度看板`（**2026-09-07 收官**：13 实现卡全 approve（B1 经两轮打回修复），四审查会话 3/3+3/3+1+1/1/1，main HEAD 6974e95f——【wave1 研究 6 张（5 回+1 裁定终止）】贴图 census（模型零缺失、缺=占位图待换真图；2785 对/40 集合勘误 37 陈旧口径；jar 资产实测在列勘误 id330「紫黑缺模」误记——真机所见=占位贴图）∥ 双预览考古（GTCEu JEI 页=ModularUI SchemaRenderer 活体 3D 非截图；GTCEu 无 keybind=世界内按键投影纯新设计；渲染路线推荐即时每帧 emission 避后台线程/Sodium 双腿坑）∥ i18n 方案（A 案 GT6ZhCn provider+TSV 数据源；dump 映射 82% 自动；B 波拼接 ~1100 预装键）∥ **移植进度看板**（常驻账本 tasks.p20-research-progress-dashboard：machines 0.9%/recipes 13%/multiblocks 5.7%/worldgen 0%/tools 4.5%…top10 缺口排序）∥ kTFRU 二轮（KortexWorker 配方执行单元/异步检查 4 安全子思路/FX 描边数据模型；机器维持池底）∥ ModularUI 审计（用户裁定终止——fork 定案无需审计）【贴图 sprint】ADR-P20 双树政策（真图静态树正典位+同路径跨树共存禁止+gen_textures skip-if-real；勘误：占位恒在静态树/fail-fast 机制/40 集合）：W1 census 守卫 0ddd034c（纯 JUnit 四断言钉）∥ W2 五卡=sets-a 75df8758（20 集合 1373 张）+sets-b 9f6fa0ae（1412 张，40 集合 2785 闭环）+machine-fronts de5b8942（bake_machine_fronts.py 泛化 P19 管线，21 张，distillery byte-identity）+tank-barrel-pipe d40d2dd8（借 10/declare 2：boiler front=BAROMETER 运行时叠加、pipe=透明截面不借）+testmachine 588571e4（2 dev 块 datagen 收官）∥ W3 tint=c 路线停手（四面核查零颜色数据源+上游 UNCOLORED 白乘=灰图原样已保真；喷涂重拆 Paintable 功能池）【i18n】ADR-P20 zh_cn 管线（provider 生成不手写+TSV 数据注入+tagprefix 手译 105）：A 波 3daf55c2（zh_cn 1999 键上线，en_us 零字节）∥ B1 1c188a52（线缆+conveyor/arm 拼接化；R1 空格回归+R2 superconductor tier 裸键两轮打回——沉淀三层正交守卫钉：槽位数/展开串全等/键面在场）∥ B2 6974e95f（石头 16 模板+rows 族收官：全量对照 513 mismatch=0 missing=0，负断言清空=拼接化收官门 PASS；en 3578→2505/zh 2089 全手译背书；materialWalkEmittedKeys 公共缝）【ModularUI】ADR-P20 fork 三裁决（上游=brachy84/ModularUI-Modern per-MC 分支；仓内 vendored third-party/modularui；LGPL-3.0 三件套）：卡① 3aacc725（vendored 657 java 单树 chisel 双腿编译绿，48% 零改勘误 ADR 90% 估算，DIVERGE.md 401 行台账）∥ 卡② 3e578845（jarJar 双腿嵌装 EvalEx/MixinExtras+生产起服冒烟双腿 Done+类加载探针实载；版本区间=裸依赖原形禁 require 漂移实证；tools/jar_content_check.py 40 项打包守卫）【终局门禁】mdk 1.20.1 全量 **1152/0/0**；21.1 compile 0 红；runData 二跑 written:0+datagen_tree_check 70851 byte-identical；分发 jar 双腿重编+jar_content_check GREEN；ADR 三篇正典（texture-tree-policy/i18n-zhcn-pipeline/modularui-fork-ruling）+勘误两则）
