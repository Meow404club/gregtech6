...
- [ ] **P16 新增池（收口移交）**：sweep SESSION_GROUPS 增 p16 簇/framework.run() node 回写一行修（GT6_SESSION=off 下 Step.node_cmds 失效）/session artifact slug 加 worktree 哈希/gt6oven run 200 基线即红追查（progress=0/0 无配方匹配，机器/配方域零 diff）/物品掩码 RCON 活体臂（side-io 离线真值表覆盖）/boiler declaration-only form 拒绝臂+creative 免费臂无 Player fixture 未单测/真实 wand 物品化（form 命令臂已兑现三件套签名）/变长机消费者（layers DSL 无生产消费者，为池件变长机备料）/GTMultiBlockPattern NOTHING 常量名补注释/fluid_sides_auto_* 出界值守卫（>5 折叠 no-op）/mExpansions HashMap 线程安全/DISPLAY_CONTAINER static SimpleContainer(0) 跨菜单共享/datagen 共享 generated 树跨节点裁决（1.21.1 runData 将 loot_tables 单数化 loot_table，共享树架构下 21.1 二跑 written:0 结构性不可达；候选=输出目录按节点参数化/per-node 产物树/接受 21.1 形落账，需架构师 ADR）
