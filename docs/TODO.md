...
- [ ] **P17 新增池（收官移交）**：GTEngineFuelsTest.loadIsIdempotentPerGeneration 活体诊断（FML 引导下 GT6RecipeMaps 静态生命周期×JUnit 方法序，NPE :154；打印方法序+每代 guard/pour 计数，禁猜修）/GT6RecipesOreChainTest.positiveControlThroughFindRecipe 活体诊断（CRUSHER null :337，同域）/sweep 结果 JSON 全局 /tmp 无 worktree 隔离（账本面互踩）/p16 八链 session 多链打包（归全集 sweep 卡，session 路径单链已活体验证）/form_scaffold 21.1 cokeoven BE load ERROR×1/遍观察（BE NBT 序列化域，Delta 3 defer）/gt6oven run 200 基线即红追查（Oven 配方注册）/物品掩码 RCON 活体臂（side-io 离线真值表覆盖）/boiler declaration-only form 拒绝臂+creative 免费臂无 Player fixture 未单测/真实 wand 物品化（form 命令臂已兑现三件套签名）/变长机消费者（layers DSL 无生产消费者，为池件变长机备料）/GTMultiBlockPattern NOTHING 常量名补注释/fluid_sides_auto_* 出界值守卫（>5 折叠 no-op）/mExpansions HashMap 线程安全/DISPLAY_CONTAINER static SimpleContainer(0) 跨菜单共享
