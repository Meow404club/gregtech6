...
- [x] **P17 新增池（收官移交）——P18 主体已落 ✅**：EngineFuels/OreChain 活体诊断 ✅（两案同根毒态，8a607b7d 世代化重置修复，合流后 **IDENTICAL-0**）/sweep JSON worktree 隔离 ✅（f315760b）/form_scaffold BE load ERROR 观察 ✅（**定性修正=P1 真实丢数据**，21ceab6e NBT_ACCESS 闭合，known_bugs closed）/gt6oven run 200 追查 ✅（verdict=链臂缺供能非 mdk 缺陷，fc494c6b 供能步修复）/GTMultiBlockPattern NOTHING 注释+fluid_sides 出界守卫+mExpansions 线程安全+DISPLAY_CONTAINER per-menu ✅（98757672）/keepFilter 读侧重建 ✅（62e2a7b1，permanent-delta 撤销）/quiet_window 自适应 ✅（f315760b）；余项顺延 P19 池：p16 八链 session 多链打包/物品掩码 RCON 活体臂/boiler declaration-only form 拒绝臂+creative Player fixture/真实 wand 物品化/变长机消费者/**W4 NBT provider 穿透（P18 beload ADR 新登）**
- [ ] **P18 新增池（收官移交）**：**W4 NBT provider 沿 Root.loadAdditional:140 穿透全 BE 树**（覆盖 saveToItem 等 level-less 保存域+getUpdateTag/saveWithoutMetadata 面+swap 表 :263-270 条款退役；冻结静态 NBT_ACCESS 缺 dynamic 注册表为全树既有暴露）/framework wait_done 8KB log_tail 窗口带病 boot 假超时（post-Done 堆栈挤出 Done 标记；预存观察）/oven 链 21.1 腿复验（B 臂无 node_cmds 分叉应兼容，归双节点 sweep 常规成员）/oven `--dual` 双腿全跑（双端已分别覆盖，全 dual 重跑不成比例）/GT6RecipeMaps reset 生产调用者出现时复核钩子链 worker 线程可见性（staticinit ADR 复核点）/vanilla rconConsoleSource upstream（DedicatedServer.java:517-521 线索，顺延）/runClient 视觉整备 backlog（dryer GUI 目视/流体面/真贴图+21.1 首次 runClient）/移植批（drying 食物·树脂·生物族行+water_hot 注册/circuit 合成·编程器交互+256 图标梯+distillery 三张 front 占位 PNG/stoneChiseled 凿石变体+TOOL_chisel 消费者族+材质梯）/curator: astchunk-src·ts-official 354 文件补审/kTFRUAddon 多方块通用化（池底）/版本扩展 1.21.4+（GT6ItemModels·GT6BlockStates）与 1.21.6+（BE ValueInput·Output）（用户：等其它全部干完再考虑）
