...
- [ ] **P16 顺延池（P15 收官移交）**：keepFilter 读侧重建（21.1 VERDICT IDENTITY LOST——0 量经 parseOptional orElse(EMPTY) 折叠，写形不可修，读侧 FluidName 重建臂=p12 forge 腿语义移植，TileEntityBase08BarrelTest:341，待架构师裁）/21.1 GTClientHandlers 抽象事件注册崩溃（GTClientHandlers.java:38 监听 RegisterColorHandlersEvent 须拆 .Item/.Block 子类；**遮蔽面=21.1 runClient 与 runData 全体**——五判据③ 1.21.1 runData 即被它阻断，修后须补 runData 双节点幂等终验）/quiet_window 自适应（gt6rcon.py:52 0.5s/条×~3000 step=RCON 提速残差大头，改此处而非并发度）/vanilla rconConsoleSource per-connection 根治（upstream 线索 DedicatedServer.java:517-521 服务器级单例并发串响应，src 域未来卡）/21.1 全套 22 红（21 例 FML 灌注反转配方链待运行时实证后逐条裁+1 例 keepFilter 真差）/1.21.4+ 回池（GT6ItemModels/GT6BlockStates item model 重做）+1.21.6+ 回池（BE ValueInput/Output 序列化层）/keepFilter 探针双节点 RETAINED/LOST 裁决/runClient 目视 backlog P15 增量（21.1 首次 runClient：JEI 19.52 面板/GTClientHandlers 修后 tint 链）
